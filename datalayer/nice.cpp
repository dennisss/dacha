...
/*
	General idea:
	- Wait for SDP
	- Parse it to credentials list and 
...
a=candidate:1600657196 1 udp 2122260223 172.31.98.228 61473 typ host generation 0 network-id 1 network-cost 10
↵a=ice-ufrag:Ydyc
↵a=ice-pwd:wRHqyRxM3UcuRYDI7ixTyZ7D
↵a=ice-options:trickle
	*/
	...
	for (item = cands; item; item = item->next) {
		...
		// TODO: sometimes a ufrag is also given
		// a=candidate:1600657196 1 udp 2122260223 172.31.98.228 61473 typ [CANDIDATE_TYPE] generation 0 network-id 1 network-cost 10

		// a=candidate:[FOUNDATION] [COMPONENT] [TRANSPORT] [PRIORITY] [IP] [PORT] typ host generation [GENERATION] network-id [NETWORK_ID] network-cost []

		
		printf(
			"a=candidate:%s %u %s %u %s %u typ %s generation %u network-id %u network-cost %u\r\n",
			c->foundation,
			c->component_id,
			c->transport == NICE_CANDIDATE_TRANSPORT_UDP? "udp" : "tcp",
			c->priority,
			ipaddr,
			nice_address_get_port(&c->addr), // TODO: Sometimes this is 0, it should be 9??
