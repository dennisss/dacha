...
///
/// All of the cluster's metadata should be accessed through this interface.
/// This interace effectively wraps each protobuf message named
/// 'cluster.*Metadata' into a table where each row has one of these messages as
/// a value.
///
/// For example, given a 'MetastoreClientInterface' named 'client', you can
/// look up a specific worker in the cluster using:
///
/// let workers_table = client.cluster_table::<WorkerMetadata>();
/// let worker = nodes_table.get("my_worker_name.1234").await?;
/// // 'worker' is a WorkerMetadata proto with worker.name() equal to the
/// // requested id.
///
/// This also supports secondary keys, so some tables like 'WorkerMetadata' also
/// store data in a hidden 'WorkerByNode' table so that users can run the
/// following to efficiently find all workers intended to be on a node:
///
/// let workers = workers_table.list_by_node(node_id).await?;
...
impl<'a> ClusterMetaTable<'a, WorkerMetadata> {
    /// Finds all workers associated with a job.
    ///
    /// This takes advantage of the fact that the names of all workers is
    /// constructed using the job name as a prefix.
    pub async fn list_by_job(&self, job_name: &str) -> Result<Vec<WorkerMetadata>> {
        self.get_prefix(&format!("{}.", job_name)).await
    }

    /// Finds all workers which are assigned to run on a specific worker.
