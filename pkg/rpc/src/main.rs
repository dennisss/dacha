...
//trait Handler<G> {
//	fn handle<'a>(&self, x: &'a mut usize) -> Pin<Box<dyn Future<Output=()> + 'a>> where G: 'a;
//}
//
...
//	fn handle<'a>(&self, x: &'a mut usize) -> Pin<Box<dyn Future<Output=()> + 'a>> where (): 'a {
//		let f = async move || {
//			*x += 4;
//		};
//
...
async fn wrapper<F>(handler: &mut F) -> usize
	where F: for<'a> AsyncFnMut1<&'a mut usize, Output=()> {
	let mut y = 1;
