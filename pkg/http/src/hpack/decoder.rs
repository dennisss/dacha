...
impl Decoder {
    ...
    fn resolve_string_ref(&self, r: StringReference, is_value: bool) -> Option<Vec<u8>> {
        match r {
            ...
            StringReference::Literal(value) => Some(value),
            StringReference::LiteralRef(value) => Some(value.to_owned()),
        }
    }
}
        ...
        while !self.input.is_empty() {
            ...
            match repr {
                HeaderFieldRepresentation::HeaderField {
                    ...
                    let name_data = self
                        .decoder
                        .resolve_string_ref(name, false)
                        .ok_or_else(|| err_msg("Failed to find name"))?;
                    let value_data = self
                        .decoder
                        .resolve_string_ref(value, true)
...
impl<'a, 'b> std::iter::Iterator for DecoderIterator<'a, 'b> {
    // TODO: We should also propagate out the 'indexed' field in case this is being
    // forwarded. TODO: Instead return referenced values?
    ...
    fn hpack_decoder_example7() -> Result<()> {
        ...
        assert_eq!(
            ...
            &[
                ...
                HeaderField {
                    name: b"date".to_vec(),
                    value: b"Mon, 21 Oct 2013 20:13:21 GMT".to_vec()
                },
                HeaderField {
                    name: b"location".to_vec(),
                    value: b"https://www.example.com".to_vec()
                },
            ]
        );
        ...
        assert_eq!(
            ...
            &[
                ...
                HeaderField {
                    name: b"date".to_vec(),
                    value: b"Mon, 21 Oct 2013 20:13:22 GMT".to_vec()
                },
                HeaderField {
                    name: b"location".to_vec(),
                    value: b"https://www.example.com".to_vec()
                ...
                HeaderField {
                    ...
                    value: b"foo=ASDJKHQKBZXOQWEOPIUAXQWEOIU; max-age=3600; version=1".to_vec()
                },
            ]
        );
