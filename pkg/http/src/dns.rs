...
pub fn lookup_hostname(name: &str) -> Result<Vec<AddrInfo>> {
	...
	while !cur_addr.is_null() {
		...
		let family = match a.ai_family {
			...
			_ => { return Err(
				format_err!("Unknown ai_family: {}", a.ai_family)); }
		};
		...
		let socket_type = match a.ai_socktype {
			...
			_ => { return Err(format_err!("Unknown ai_socktype: {}", a.ai_socktype)); }
		};
		...
		let address = match unsafe { *a.ai_addr }.sa_family as i32 {
			libc::AF_INET => {
				let addr_in = unsafe {
					*std::mem::transmute::<*const libc::sockaddr,
			...
			_ => { return Err(format_err!("Unsupported family in ai_addr")); }
		};
