...
pub fn lookup_hostname(name: &str) -> Result<Vec<AddrInfo>> {
	...
	let ret = unsafe {
		getaddrinfo(cname.as_ptr(), null(), null(), &mut addrs) 
	};

	// TODO: Use gai_strerror to print the error?
	if ret != 0 {
		return Err(format!("Got error {}", ret).into());
	}
	...
	while !cur_addr.is_null() {
		...
		let family = match a.ai_family {
			...
			_ => { return Err(
				format!("Unknown ai_family: {}", a.ai_family).into()); }
		};
		...
		let socket_type = match a.ai_socktype {
			...
			_ => { return Err(format!("Unknown ai_socktype: {}", a.ai_socktype).into()); } 
		};
		...
		let address = match unsafe { *a.ai_addr }.sa_family as i32 {
			libc::AF_INET => {
				let addr_in = unsafe {
					*std::mem::transmute::<*const libc::sockaddr, *const libc::sockaddr_in>(a.ai_addr) };
			...
			libc::AF_INET6 => {
				let addr_in6 = unsafe {
					*std::mem::transmute::<*const libc::sockaddr, *const libc::sockaddr_in6>(a.ai_addr) };
				
				IPAddress::V6(addr_in6.sin6_addr.s6_addr.to_vec())
				
			},
			_ => { return Err(format!("Unsupported family in ai_addr").into()); }
		};
	...
	unsafe { libc::freeaddrinfo(addrs); };
