		...
		match self.state.clone() {
			ChunkState::Start => {
				let line = match self.stream.read_matching(LineMatcher::any()).await {
					...
					Err(_) => { return Err(err_msg("IO error while reading chunk start line")); },
					_ => { return Err(err_msg("Expected chunk start line")); }
				};

				let (head, _) = match complete(parse_chunk_start)(line) {
					Ok(v) => v,
					_ => { return Err(err_msg("Invalid chunk start line")); }
				};
			...
			ChunkState::Trailer => {
				let data = match self.stream.read_matching(LineMatcher::empty()).await {
					Ok(StreamReadUntil::Value(v)) => v,
					Err(_) => { return Err(err_msg("io error while reading trailer")); },
					_ => { return Err(err_msg("Expected trailer empty line")); }
				};

				let (headers, _) = match complete(parse_chunk_end)(data) {
					Ok(v) => v,
					_ => { return Err(err_msg("Invalid chunk trailer")); }
				};
