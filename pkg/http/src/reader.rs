...
impl LineMatcher {
	pub fn any() -> Self {
		LineMatcher { seen_cr: false, cur_length: 0, empty_only: false }
	}

	pub fn empty() -> Self {
		LineMatcher { seen_cr: false, cur_length: 0, empty_only: true }
	}
}
...
impl StreamReader {
	...
	// TODO: If this ends up being called too many times, then the total amount of memory used by a single connection may get very large.
	pub async fn read_matching<M: Matcher>(&mut self, mut matcher: M)
...
// 	pub fn read_exact(&mut self, mut buf: &mut [u8]) -> impl common::FutureResult<()> {
// 		self.stream.lock().unwrap().read_exact(buf)
// 	}
// }
