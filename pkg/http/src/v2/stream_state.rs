...
pub struct StreamState {
    ...
    /// closed.
    ///
    /// This corresponds to either a local/remote RST_STREAM or GOAWAY frame
    /// being sent.
    pub error: Option<ProtocolErrorV2>,

    /// Number of bytes of data the local endpoint is willing to accept from the
    ...
    ///
    /// TODO: Make this a cyclic buffer or a list of chunked buffers. (the
    /// challenge with a cyclic buffer is that we should block accidentally
    /// overriding data)
    pub received_buffer: Vec<u8>,

    pub received_trailers: Option<Headers>,

    /// If true, aside from what is in 'received_buffer' and
    /// 'received_trailers', we have received all data on this stream from
    /// the remote endpoint.
    ///
    ...
    /// Total number of bytes that we've received from the remote endpoint on
    /// this stream.
    ...
    /// use remote_window as the max for this as that may be an insanely
    /// large number)
    ...
    /// This is copied from the ConnectionOptions::max_sending_buffer_size at
    /// the time of stream creation.
