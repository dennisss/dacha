...
impl Stream {
    ...
    fn set_received_end(&self, end_stream: bool, state: &mut StreamState) -> StreamResult<()> {
        if state.received_end {
            return Err(StreamError::stream_closed("Received data after stream was closed"));
        }
    ...
    fn increment_received_bytes(&self, increment: usize, state: &mut StreamState) -> StreamResult<()> {
        if let Some(expected_size) = state.received_expected_bytes.clone() {
            ...
            state.received_total_bytes = state.received_total_bytes.checked_add(increment)
                .ok_or_else(|| StreamError::malformed_message("Overflow in received number of bytes"))?;
    ...
    pub fn receive_request(
        &self, headers: Vec<hpack::HeaderField>, end_stream: bool,
        incoming_body: IncomingStreamBody, state: &mut StreamState
    ) -> Option<Request> {
        if state.error.is_some() {
            return None;
        }
    ...
    pub fn receive_response(
        &self, request_method: Method, headers: Vec<hpack::HeaderField>, end_stream: bool,
    ...
    // TODO: We must ensure that these functions immediately store an error in the stream state
    // so that we don't run into any race conditions with the incoming/outgoing body checking
    // the error in between locking cycles.

    pub fn receive_data(&self, data: &[u8], extra_flow_controlled_bytes: usize, end_stream: bool, state: &mut StreamState) {
        if state.error.is_some() {
            return;
        }

        if let Err(StreamError(error)) = self.receive_data_inner(data, extra_flow_controlled_bytes, end_stream, state) {
    ...
    /// Called whenever we successfully received a DATA or trailing HEADERS frame.
    ///
    /// If this fails, then the stream needs to be reset with the returned error.
    fn receive_data_inner(&self, data: &[u8], extra_flow_controlled_bytes: usize, end_stream: bool, state: &mut StreamState) -> StreamResult<()> {
        self.set_received_end(end_stream, state)?;
        
        if self.incoming_response_handler.is_some() {
            return Err(StreamError::malformed_message("Expected response headers before stream data"));
        }
    ...
    pub fn receive_trailers(
        &self, trailers: Vec<hpack::HeaderField>, end_stream: bool,
        state: &mut StreamState
    ) {
        if state.error.is_some() {
            return;
    ...
    fn receive_trailers_inner(
        ...
        if self.incoming_response_handler.is_some() {
            return Err(StreamError::malformed_message("Expected response headers before trailer headers"));
        }

        // NOTE: This may fail as we need to verify '==' of the received bytes once the stream
        // has ended.
