...
pub struct ConnectionState {
    ...
    /// This is mainly used to ensure that at most one set of reader/writer
    /// threads are associated with the connection.
    pub running: bool,
    ...
    // streams?? (e.g. no more requests or PUSH_PROMISES??)
    ...
    /// If a request is still in this list then it definately hasn't been sent
    /// to the other endpoint yet.
    ///
    /// On a client, the total number of outstanding requests is:
    ///     'pending_requests.len() + local_open_stream_count'
    ///
    /// TODO: If the client side hands up on a request either before it was sent
    /// or before we were done receiving the response we need to ensure that
    ...
    /// Next value of 'local_settings' which is pending acknowledgement from the
    /// other endpoint.
    ...
    /// Whether or not we have received some set of settings from the remote
    /// endpoint (e.g. the HTTP2-Settings header or an initial SETTINGS
    /// frame) Else, the remote_settings field will only contain the default
    /// settings defined by the HTTP2 protocol.
    ...
    /// The highest stream id which we will accept from the remote endpoint. By
    /// default this will be MAX_STREAM_ID, but will be decreased during
    /// graceful shutdown.
...
pub enum ConnectionEvent {
    /// We received a ping from the remote endpoint. In response, we should
    /// respond with an ACK.
    ///
    ...
    /// A locally initialized GOAWAY was triggered. In response, we should let
    /// the other endpoint know about it.
    // Goaway {
    ...
    /// 1. All streams are closed, so error == None and we are done gracefully
    /// shutting down. 2. We received a remote GOAWAY with a non-NO_ERROR
    /// code or the reader thread failed, so we    should close the
    /// connection ASAP.

    /// NOTE: If you send this event, you are responsible for setting
    /// ConnectionState::shutting_down and
    ...
    /// We received remote settings which we've applied to the local state and
...
pub enum ShuttingDownState {
    ...
    Graceful {
        ...
        /// state. Only used on Server endpoints to ensure that the
        /// server shutdown time is bounded.
