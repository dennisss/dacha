...
impl LabelManager {
    ...
    pub async fn open_with_context(context: usb::Context) -> Result<Self> {
        ...
        // NOTE: Can't be changed while the kernel is also holding the second interface
        // for mass storage.
    ...
    pub async fn read_status(&self) -> Result<Status> {
        ...
        self.device
            .write_interrupt(USB_EP_OUT, &status_request)
            .await?;

        let mut status_response = [0u8; STATUS_LENGTH];
        let n = self
            .device
            .read_interrupt(USB_EP_IN, &mut status_response)
            .await?;
    ...
    /// TODO: We need to lock the device in order to send large multi-packet
    /// payloads.
