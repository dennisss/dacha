...
#[cfg(feature = "std")]
pub trait FutureResult<T> = core::future::Future<Output = base_error::Result<T>>;
...
macro_rules! enum_accessor {
    ($name:ident, $branch:ident, $t:ty) => {
        fn $name(&self) -> $crate::base_error::Result<$t> {
            if let Self::$branch(v) = self {
                Ok(*v)
            } else {
                Err($crate::base_error::err_msg("Unexpected value type."))
            }
        }
    };
}
...
/// Verifies
#[cfg(feature = "alloc")]
pub fn check_zero_padding(data: &[u8]) -> base_error::Result<()> {
    for byte in data.iter() {
        if *byte != 0 {
            return Err(base_error::err_msg("Non-zero padding seen"));
        }
    }
