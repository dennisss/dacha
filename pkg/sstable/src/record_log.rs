    ...
    fn parse(input: &'a [u8]) -> Result<(Self, usize)> {
        if input.len() < (RECORD_HEADER_SIZE as usize) {
            return Err(std::io::Error::new(
                ...
                "Input shorter than record header",
            )
            .into());
        }
...
pub struct RecordReader {
    ...
    /// If this is non-empty, then we have already received a FIRST record and
    /// we are currently waiting on MIDDLE|LAST records.
...
impl RecordReader {
    ...
    async fn read_record<'a>(&'a mut self) -> Result<Option<Record<'a>>> {
        // When there are < RECORD_HEADER_SIZE bytes remaining in the block, we know
        // that they will always be padding so we should advance to the next
        // block.
        //
        // Otherwise we'll just ensure that the current block is fully read (at least up
        ...
        if BLOCK_SIZE as usize - self.block_offset < RECORD_HEADER_SIZE as usize {
            ...
            // NOTE: If we had previously hit the end of the file, then this will act to
            // check if more bytes have become available in the file.
            //
            // In the case of us not expecting any new bytes later on, then this may make
            // reading of a incomplete final block in the file very expensive as
            // read() will be called for every single record in the block.
            //
            // TODO: Consider only calling this if the block size is 0, or we just returned
            // a None in the previous call to read_record().
            self.read_block_remainder().await?;
        }
    ...
    /// NOTE: Values of Ok(None) can be retried later safely if the file grew in
    /// size.
    ...
    /// Call after you think you've fully read the entire file to verify that
    /// there are no more unread bytes after the current position (only use
    /// this if you know for sure that the file won't be modified in the
    /// near future).
...
impl RecordWriter {
    ...
    */
    ...
    // not partially written. If it is, then consider truncating or skipping one
    // block ahead. TODO: Verify this code
    ...
    pub async fn append(&mut self, data: &[u8]) -> Result<()> {
        ...
        while pos < data.len() {
            // TODO: Check for overflow although that should never happen if we did
            // everything right.
