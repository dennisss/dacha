...
impl<'a> InternalKey<'a> {
	...
	pub fn before(user_key: &'a [u8]) -> Self {
		Self { user_key, sequence: MAX_SEQUENCE, typ: VALUE_FOR_SEEK }
	}
...
impl InternalKeyComparator {
	pub fn wrap(user_key_comparator: Arc<dyn Comparator>) -> Arc<Self> {
		Arc::new(Self { user_key_comparator })
	}
}
...
impl Comparator for InternalKeyComparator {
	...
	fn compare(&self, a: &[u8], b: &[u8]) -> Ordering {
		...
		match self.user_key_comparator.compare(a_ik.user_key, b_ik.user_key) {
	...
	fn find_shortest_separator(&self, mut start: Vec<u8>, limit: &[u8])
							   -> Vec<u8> {
		...
		start = self.user_key_comparator.find_shortest_separator(
			start, InternalKey::user_key(limit));
...
impl InternalKeyFilterPolicy {
	pub fn wrap(user_key_filter_policy: Arc<dyn FilterPolicy>)
		-> Arc<dyn FilterPolicy> {
		Arc::new(Self { user_key_filter_policy })
	}
}

impl FilterPolicy for InternalKeyFilterPolicy {
	fn name(&self) -> &'static str { self.user_key_filter_policy.name() }
	...
	fn key_may_match(&self, key: &[u8], filter: &[u8]) -> bool {
		self.user_key_filter_policy.key_may_match(InternalKey::user_key(key),
												  filter)
	}
}
