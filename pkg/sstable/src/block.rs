...
/// the metaindex block, and the index block.
#[derive(Debug)]
pub struct Block<'a> {
	/// Starting at the beginning of the block, how many bytes are used for
	/// key-value entries.
	pub entries: &'a [u8],
...
impl<'a> Block<'a> {
	pub fn parse(mut input: &'a [u8]) -> Result<Self> {
		...
		let index_type =
			if (packed >> 31) == 0 { BlockIndexType::BinarySearch }
			else { BlockIndexType::BinaryAndHash };
		...
		let hash_index =
			if index_type == BlockIndexType::BinaryAndHash {
				min_size!(input, 1);
		...
		Ok(Self { entries: input, hash_index, restarts })
	}
	...
	pub fn iter(&'a self) -> BlockEntryIterator<'a> {
		BlockEntryIterator { remaining_entries: self.entries }
	}
	...
	fn restart_search(&self, key: &[u8], restarts: &[u32]) -> Result<usize> {
		...
		match key.cmp(mid_entry.key_delta) {
			...
			Ordering::Greater => self.restart_search(key,
													 &restarts[mid_index..])
		}
	}
}
...
impl<'a> BlockEntry<'a> {
	pub fn parse(mut input: &'a [u8]) -> Result<(Self, &'a [u8])> {
		...
		Ok((
			Self { shared_bytes: shared_bytes as u32, key_delta, value },
			input
		))
	}
...
impl<'a> BlockEntryIterator<'a> {
	pub fn rows(self) -> BlockKeyValueIterator<'a> {
		BlockKeyValueIterator { inner: self, last_key: vec![] }
	}
}
...
impl<'a> BlockKeyValueIterator<'a> {
	pub fn next<'b>(&'b mut self) -> Option<Result<KeyValuePair<'b>>> {
		...
		Some(Ok(KeyValuePair { key: &self.last_key, value: entry.value }))
	}
}
