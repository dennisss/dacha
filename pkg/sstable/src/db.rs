...
use crate::table_builder::{SSTableBuilderOptions, SSTableBuilder};
...
//	manifest_file.read_to_end(&mut manifest_data).await?;
		...
		let next_file_number = AtomicU64::new(
			version_edit.next_file_number.unwrap_or(2));
		...
		let mut mutable_table = MemTable::new(
			options.table_options.comparator.clone()
		);
		...
		Ok(Self {
			options, dir, lock_file, identity, next_file_number,
			state: RwLock::new(EmbeddedDBState {
				prev_log_number, log_number, log,
				mutable_table,
				immutable_table,
				level_tables: vec![]
			})
...
impl EmbeddedDBOptions {
	pub fn wrap_internal_keys(mut self) -> Self {
		self.table_options.comparator = InternalKeyComparator::wrap(
			self.table_options.comparator);
		self.table_options.filter_policy = self.table_options.filter_policy.map(
			|policy| InternalKeyFilterPolicy::wrap(policy));
		self
	}
}
...
impl FilePaths {
	...
	pub fn identity(&self) -> PathBuf { self.root_dir.join("IDENTITY") }
