...
impl SSTableBuilder {
	pub async fn open(path: &Path, options: SSTableBuilderOptions)
		-> Result<Self> {
		// NOTE: We will panic if we are overwriting an existing table. This
		// should never
		let mut file = OpenOptions::new().write(true)
			.create_new(true).open(path).await?;

		let filter_block_builder =
		...
		let data_block_builder =
			BlockBuilder::new(options.block_restart_interval);
	...
	async fn write_raw_block(&mut self, block_buffer: &mut Vec<u8>)
		-> Result<BlockHandle> {
	...
	pub async fn add(&mut self, key: Vec<u8>, value: Vec<u8>) -> Result<()> {
		let min_cutoff = (self.options.block_size *
			self.options.block_size_deviation) / 100;

		// If we expect to overflow the block size, flush the flush the previous
		// block.
		// TODO: Check the edge cases of min_cutoff inequality.
		if (self.data_block_builder.projected_size(&key, &value)
