...
pub const USBDEVFS_IOC_DISCONNECT: libc::c_int = request_code_none!(USBDEVFS_IOC_MAGIC, 22) as libc::c_int;
pub const USBDEVFS_IOC_CONNECT: libc::c_int = request_code_none!(USBDEVFS_IOC_MAGIC, 23) as libc::c_int;
...
// TODO: This is only thread safe if we access it after the linux kernel reaps it.
#[derive(Debug)]
#[repr(C)]
...
ioctl_write_ptr!(usbdevfs_getdriver_fn, USBDEVFS_IOC_MAGIC, 8, usbdevfs_getdriver);
ioctl_read!(usbdevfs_setinterface_fn, USBDEVFS_IOC_MAGIC, 4, usbdevfs_setinterface);
ioctl_read!(usbdevfs_claim_interface, USBDEVFS_IOC_MAGIC, 15, libc::c_uint);
ioctl_read!(usbdevfs_release_interface, USBDEVFS_IOC_MAGIC, 16, libc::c_uint);
ioctl_read!(usbdevfs_submiturb, USBDEVFS_IOC_MAGIC, 10, usbdevfs_urb);
ioctl_write_ptr_bad!(usbdevfs_discardurb, request_code_none!(USBDEVFS_IOC_MAGIC, 11), usbdevfs_urb);
ioctl_write_ptr!(usbdevfs_reapurb, USBDEVFS_IOC_MAGIC, 12, &usbdevfs_urb);
ioctl_write_ptr!(usbdevfs_reapurbndelay, USBDEVFS_IOC_MAGIC, 13, *const usbdevfs_urb);
ioctl_readwrite!(usbdevfs_ioctl_fn, USBDEVFS_IOC_MAGIC, 18, usbdevfs_ioctl);
ioctl_none!(usbdevfs_reset, USBDEVFS_IOC_MAGIC, 20);
ioctl_read!(usbdevfs_setconfiguration, USBDEVFS_IOC_MAGIC, 5, libc::c_uint);