...
///
/// Internally this runs a single background thread for polling all of the devices created
/// using the same context. 
///
/// Drop semantics:
/// - The context will be cleaned up when all references to the context are dropped. Meaning:
...
///   - Internally the Context and every open Device contains a Arc<ContextState>
pub struct Context {
...
pub struct ContextState {
    ...
    /// File descriptor for the eventfd() used to notify the background thread when a change
    /// to the open devices has occured.
    background_thread_eventfd: libc::c_int,

    /// The sending end of the set of all channels which are waiting for the background thread
    /// to finish executing at least one cycle.
...
impl Context {
    pub fn create() -> Result<Arc<Self>> {
        let background_thread_eventfd = unsafe {
            libc::eventfd(0, libc::O_CLOEXEC | libc::O_NONBLOCK)
        };
        ...
        *instance.state.background_thread_handle.lock().unwrap() = Some(thread::spawn(
            move || Self::run_background_thread(background_state)));

        Ok(instance)
    }

    // TODO: Don't make this public! When this is called, it won't destroy the devices so must only
    // be called after all devices are destroyed.
    fn close(&mut self) {
        // TODO: Check return value
        unsafe { libc::close(self.state.background_thread_eventfd) };

        // NOTE: The background thread should terminate itself shortly now that the eventfd is dead.
    }
        ...
        loop {
            ...
            for fd in &fds {
                ...
                if fd.fd == context_state.background_thread_eventfd {
                    ...
                    let n = unsafe { libc::read(fd.fd, std::mem::transmute(&mut event_num), std::mem::size_of::<u64>()) };
    ...
    pub(crate) fn notify_background_thread(&self) -> Result<()> {
        ...
        let event_num: u64 = 1;
        let n = unsafe {
            libc::write(self.state.background_thread_eventfd, std::mem::transmute(&event_num), std::mem::size_of::<u64>())
        };
        if n != (std::mem::size_of::<u64>() as isize) {
            return Err(err_msg("Failed to notify background thread"));
        }

        // TODO: Ignore EAGAIN errors. Mains that the counter overflowed (meaning that it already has a value set.)
    ...
    ///
    /// Internally this uses sysfs for similar reasons to libusb. In particular this enables us to
    /// use cached kernel device descriptors rather than opening each device. 
    pub async fn enumerate_devices(self: &Arc<Self>) -> Result<Vec<DeviceEntry>> {
        ...
        while let Some(res) = entries.next().await {
            ...
            let path = entry.path();
            let file_name = path.file_name().map(|s| s.to_str().unwrap_or("")).unwrap_or_default();
    ...
    async fn enumerate_single_device(self: &Arc<Self>, sysfs_dir: &Path) -> Result<DeviceEntry> {
        let busnum = fs::read_to_string(sysfs_dir.join("busnum")).await?
            .trim_end()
            .parse::<usize>()?;
        
        let devnum = fs::read_to_string(sysfs_dir.join("devnum")).await?
            .trim_end()
            .parse::<usize>()?;
...
/// Can be used to open the device or preview descriptors that are cached by the system.
...
impl DeviceEntry {
    pub fn device_descriptor(&self) -> Result<DeviceDescriptor> {
        ...
        match iter.next() {
            ...
            _ => {
                Err(err_msg("Expected first cached descriptor to be a device descriptor"))
            }
        }
    }
    ...
    pub async fn open(&self) -> Result<Device> {
        ...
        let state = Arc::new(DeviceState {
            bus_num: self.busnum,
            dev_num: self.devnum,
            fd, transfers: std::sync::Mutex::new(DeviceStateTransfers::default())
        });
        ...
        Ok(Device {
            ...
            context: self.context.clone(), state, closed: false })
    }
}
...
impl<'a> DescriptorIter<'a> {
    fn next_impl(&mut self) -> Result<Option<Descriptor>> {
        ...
        fn decode_fixed_len_desc<T: Copy>(raw_desc: &[u8]) -> Result<T> {
            ...
            // TODO: This transmute assumes that we are running on a little-endian system (same as
            // the wire endian of the USB descriptors).
            Ok(*unsafe { std::mem::transmute::<_, &T>(raw_desc.as_ptr()) })
        }

        Ok(Some(match typ {
            Some(DescriptorType::DEVICE) => {
                Descriptor::Device(decode_fixed_len_desc(raw_desc)?)
            }
