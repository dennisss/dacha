...
pub struct HIDDevice {
    ...
    /// Interrupt input endpoint which is used to receive reports asynchronously.
    in_endpoint: u8,
...
impl HIDDevice {
    pub async fn open_with_existing(mut device: Device) -> Result<Self> {
        if device.descriptor().bNumConfigurations != 1 {
            return Err(err_msg("Currently only devices with one config are supported."))
        }

        // TODO: Validate in the USB driver that there are no duplicate interface numbers.
        ...
        for desc in device.descriptors() {
            match desc {
                ...
                Descriptor::Endpoint(ep) => {
                    let iface_key = current_iface.clone()
        ...
        if hid.bReportDescriptorType != HIDDescriptorType::Report as u8 {
            return Err(err_msg("Expected first HID descriptor to be the Report descriptor"));
        }
        ...
        let endpoints = endpoints_per_iface.remove(&(iface, alt_setting)).unwrap_or_default();
        let mut in_endpoint = None;
        let mut out_endpoint = None;

        for endpoint in endpoints {
            if endpoint.bmAttributes & 0b11 != 0b11 {
                return Err(err_msg("Expected only interrupt endpoints on HID interface"));
            }
        ...
        let in_endpoint = in_endpoint
            .ok_or_else(|| err_msg("HID interface missing required input endpoint"))?;
    ...
    pub async fn set_report(&self, report_id: u8, report_type: ReportType, data: &[u8]) -> Result<()> {
        let extended_data = {
            if (report_id == 0) != self.report_ids.is_empty() {
                return Err(err_msg("A report id of 0 must be specified iff there are no reports configured"));
            }
    ...
    pub async fn get_report(&self, report_id: u8, report_type: ReportType, data: &mut [u8]) -> Result<()> {
        if (report_id == 0) != self.report_ids.is_empty() {
            return Err(err_msg("A report id of 0 must be specified iff there are no reports configured"));
        }
