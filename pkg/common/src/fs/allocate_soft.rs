...
// - This provides a version of fs2's allocate() that does not modify the size
//   of the file's contents (but only the allocated space)
// - NOTE: Currently this just contains the linux and mac implementations
// - NOTE: This is also optimized for the append-only case, such that if the
//   given size is less than the current size of the file, then we won't bother
//   trying to make is smaller
