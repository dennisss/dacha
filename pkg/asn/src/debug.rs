...
impl ParsedElement {
    fn parse(input: Bytes) -> ParseResult<Self> {
        ...
        let value = if el.ident.constructed {
            ...
            match el.ident.tag.number {
                TAG_NUMBER_BOOLEAN => ParsedElementValue::Bool(bool::from_der(el.outer)?),
                TAG_NUMBER_INTEGER => ParsedElementValue::Int(BigInt::from_der(el.outer)?),
                TAG_NUMBER_NULL => {
                    Null::from_der(el.outer)?;
                    ParsedElementValue::Null
                }
                TAG_NUMBER_BIT_STRING => {
                    ParsedElementValue::Bits(BitString::from_der(el.outer)?.into())
                }
                TAG_NUMBER_OCTET_STRING => {
                    ParsedElementValue::Binary(OctetString::from_der(el.outer)?.into_bytes())
                }
                TAG_NUMBER_UTF8STRING => {
                    ParsedElementValue::String(UTF8String::from_der(el.outer)?.to_string())
                }
                TAG_NUMBER_OBJECT_IDENTIFIER => {
                    ParsedElementValue::ObjectIdentifier(ObjectIdentifier::from_der(el.outer)?)
                }
