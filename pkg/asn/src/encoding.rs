...
impl DERReader {
    ...
    fn read_any_element(&mut self) -> Result<Element> {
        ...
        let el: Result<Element> = match &mut self.remaining {
            DERReaderBuffer::Empty => Err(err_msg("No remaining data in reader")),
            ...
            DERReaderBuffer::Parsed(map) => Err(err_msg("Can't read ANY in set.")),
        };
    ...
    fn read_element(&mut self, class: TagClass, number: usize, constructed: bool) -> Result<Bytes> {
    ...
    pub fn read_implicitly<T, F: FnMut(&mut DERReader) -> T>(&mut self, tag: Tag, mut f: F) -> T {
    ...
    pub fn read_choice<T, F: FnMut(&mut DERReader) -> Result<T>>(&mut self, mut f: F) -> Result<T> {
    ...
    pub fn read_bool(&mut self) -> Result<bool> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_BOOLEAN, false)?;
    ...
    pub fn read_int(&mut self) -> Result<BigInt> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_INTEGER, false)?;
    ...
    pub fn read_bitstring(&mut self) -> Result<BitString> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_BIT_STRING, false)?;
        if data.len() < 1 {
        ...
        let data = BitVector::from(&data[1..], 8 * (data.len() - 1) - (nunused as usize));
        Ok(BitString { data })
    }

    pub fn read_octetstring(&mut self) -> Result<OctetString> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_OCTET_STRING, false)?;
        Ok(OctetString(data.into()))
    }

    pub fn read_null(&mut self) -> Result<()> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_NULL, false)?;
        if data.len() != 0 {
    ...
    pub fn read_oid(&mut self) -> Result<ObjectIdentifier> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_OBJECT_IDENTIFIER, false)?;
    ...
    pub fn read_utf8string(&mut self) -> Result<UTF8String> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_UTF8STRING, false)?;
    ...
    pub fn read_sequence<T, F: Fn(&mut DERReader) -> Result<T>>(
        ...
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_SEQUENCE, true)?;
    ...
    pub fn read_sequence_of<T: DERReadable>(&mut self) -> Result<Vec<T>> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_SEQUENCE, true)?;
    ...
    pub fn read_set<T, F: Fn(&mut DERReader) -> Result<T>>(
        ...
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_SET, true)?;
        ...
        for i in 1..elements.len() {
            if elements[i - 1].ident.tag >= elements[i].ident.tag {
                return Err(err_msg("Set elements not sorted or have duplicate tags"));
            }
        }
    ...
    pub fn read_printable_string(&mut self) -> Result<AsciiString> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_PRINTABLE_STRING, false)?;
        for b in &data {
    ...
    pub fn read_t61_string(&mut self) -> Result<String> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_T61STRING, false)?;
    ...
    pub fn read_universal_string(&mut self) -> Result<String> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_UNIVERSALSTRING, false)?;
    ...
    pub fn read_bmp_string(&mut self) -> Result<BMPString> {
        let data = self.read_element(TagClass::Universal, TAG_NUMBER_BMPSTRING, false)?;
...
impl<'a> DERWriter<'a> {
    ...
    // TODO: It is important to ensure that write_implicitly and write_explicitly
    // are only only used to write a single element in the callback.

    pub fn write_implicitly<F: FnMut(&mut DERWriter)>(&mut self, tag: Tag, mut f: F) {
        if self.implicit_tag.is_none() {
            self.implicit_tag = Some(tag);
        }
    ...
    pub fn write_explicitly<F: FnMut(&mut DERWriter)>(&mut self, tag: Tag, mut f: F) {
        let mut data = vec![];
        {
    ...
    pub fn write_oid(&mut self, oid: &ObjectIdentifier) {
        ...
        self.write_tag(TagClass::Universal, TAG_NUMBER_OBJECT_IDENTIFIER, false);
        self.write_length(data.len());
        self.out.extend_from_slice(&data);
    }
    ...
    pub fn write_generalized_time(&mut self, time: &GeneralizedTime) {
        self.write_tag(TagClass::Universal, TAG_NUMBER_GENERALIZEDTIME, false);
        let s = time.to_string();
        self.write_length(s.as_bytes().len());
        self.out.extend_from_slice(s.as_bytes());
    }
    ...
    pub fn write_universal_string(&mut self, s: &str) {
        ...
        self.write_tag(TagClass::Universal, TAG_NUMBER_UNIVERSALSTRING, false);
        self.write_length(data.len());
        self.out.extend_from_slice(&data);
    }
    ...
    pub fn write_bmp_string(&mut self, s: &str) {
        ...
        self.write_tag(TagClass::Universal, TAG_NUMBER_BMPSTRING, false);
        self.write_length(data.len());
        self.out.extend_from_slice(&data);
    }
...
impl DERReadable for UTF8String {
    fn read_der(r: &mut DERReader) -> Result<Self> {
        r.read_utf8string()
    }
}
...
impl DERReadable for UniversalString {
    fn read_der(reader: &mut DERReader) -> Result<Self> {
        Ok(Self {
            data: reader.read_universal_string()?,
        })
    }
}
...
impl Any {
    ...
    pub fn parse_as<T: DERReadable>(&self) -> Result<T> {
        let mut reader = DERReader::from_buffer(DERReaderBuffer::Single(self.element.clone()));
        let out = T::read_der(&mut reader)?;
        reader.finished()?;
        Ok(out)
    }

    // pub const fn from_static(v: &'static (dyn DERWriteable + Send + Sync)) ->
    // Self { 	Self::Reference(v)
    // }
}
...
impl DERWriteable for Any {
    fn write_der(&self, writer: &mut DERWriter) {
        ...
        writer.write_tag(
            self.element.ident.tag.class,
            self.element.ident.tag.number,
            self.element.ident.constructed,
        );
