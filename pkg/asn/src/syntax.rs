...
pub struct ComponentBody {
    pub types: Vec<ComponentType>, /* TODO: Add extension index and */
...
pub enum CharacterStringType {
    Restricted(RestrictedCharacterStringType),
    Unrestricted,
}
...
mod tests {
    ...
    fn asn1_syntax_test() {
        // let (v, _) =
        // complete(Constraint::parse)(Bytes::from("(SET(1..3))")).unwrap();
        // println!("{:?}", v);
        ...
        let mut file = std::fs::File::open(
            "/home/dennis/workspace/dacha/third_party/pkix/src/PKIX1Explicit88.asn1",
        )
        .unwrap();
        let mut data = vec![];
        file.read_to_end(&mut data).unwrap();

        let (module, _) = complete(ModuleDefinition::parse)(Bytes::from(data)).unwrap();
