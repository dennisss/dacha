...
// - All fields in a set have distinct tags (after applying any automatic
//   tagging if enables)
...
impl FileCompiler {
    ...
    fn compile_enumerated(&self, name: &str, typ: &EnumeratedType) -> Result<LineBuilder> {
        self.compile_int_enum(name, &typ.0.items, true)
    }

    fn compile_collection_type(
        &self,
        ctype: &CollectionType,
        ctx: &Context,
    ) -> Result<(String, LineBuilder)> {
        match ctype {
            CollectionType::Named(t) => self.compile_type(t.name.as_ref(), &t.typ, ctx),
            CollectionType::Type(t) => self.compile_type("Item", &t, ctx),
        }
    }

    fn compile_type_prefixes(
        &self,
    ...
    fn compile_struct(
        ...
        impl_der_readable(&mut lines, name, |l| {
            ...
            l.indented(|l| {
                ...
                for f in fields.iter() {
                    ...
                    field_lines.add(format!("{}::read_der(r_)", typename.replace("<", "::<")));

                    // TODO: Must also implement good support for options?

                    // TODO: This is still a major problem.
                    // TODO: This is problematic for things like structs in structs which should
                    // sometimes be handling their own prefixes.
                    ...
                    if let ComponentMode::Optional = f.mode {
                        field_lines.indent();
                        field_lines.wrap_with("r_.read_option(|r_| {".into(), "})?;".into());
                        ...
                        let builtin_type = match &f.typ.desc {
                            ...
                            TypeDesc::Referenced(v) => self.lookup_type(v.as_ref()).unwrap(),
                        };
                        ...
                        // TODO: For all constants, we might as well pre-serialize them in the
                        // compiled version?

                        field_lines.wrap_with(
                            "r_.read_with_default(|r_| {".into(),
                            format!("}}, {}.clone().into())?;", valuec),
                        );

                    // format!("}})?.unwrap_or({}.clone().into());",
                    //									valuec)

                    // TODO: We can't use der_eq because things like 'bool' have
            ...
            self.compile_type_prefixes(EncodingMode::Read, prefixes, l)
                .unwrap();
        });
        lines.nl();
        ...
        impl_der_writeable(&mut lines, name, |l| {
            ...
            l.indented(|l| {
                for f in fields.iter() {
                    ...
                    if let ComponentMode::Optional = &f.mode {
                        ...
                        field_lines
                            .add(format!("if !der_eq(&self.{}, &{}) {{", field_name, valuec));
                        // TODO: This line is the same as the default case.
                        field_lines.add(format!("\tself.{}.write_der(w_);", field_name));
                        field_lines.add("}");
                    } else {
                        field_lines.add(format!("self.{}.write_der(w_);", field_name));
                    }
            ...
            self.compile_type_prefixes(EncodingMode::Write, prefixes, l)
                .unwrap();
        });
    ...
    fn compile_wrapped_type(&self, name: &str, typ: &Type) -> Result<LineBuilder> {
        let mut l = LineBuilder::new();

        let modname = name.to_ascii_lowercase();
        let inner_ctx = Context::new().inner(&modname);

        let (typename, mut outer) = self.compile_type("Value", typ, &inner_ctx)?;
                ...
                if t.named_bits.len() > 0 {
                    ...
                    for bit in &t.named_bits {
                        let v = match &bit.value {
                            ...
                            NamedBitValue::Defined(v) => Self::value_name(v.as_ref()),
                        };
...

        {
            ...
            l.add(format!("\tfn as_ref(&self) -> &{} {{", reftype));
            l.add(format!(
                "\t\t&self.value{}",
        ...
        impl_der_writeable(&mut l, name, |l| {
            ...
            self.compile_type_prefixes(EncodingMode::Write, &typ.prefixes, l)
                .unwrap();
        });
        l.nl();
        ...
        impl_der_readable(&mut l, name, |l| {
            l.add(format!(
                "{}::read_der(r_).map(|value| Self {{ value }})",
                typename.replace("<", "::<")
            ));
            self.compile_type_prefixes(EncodingMode::Read, &typ.prefixes, l)
                .unwrap();
        });
        l.nl();
    ...
    /// TODO: Instead of accepting a Type, accept only a BuiltinType after being
    /// resolved.
    fn compile_type(
        &self,
        original_name: &str,
                ...
                String::from(match t.as_ref() {
                    ...
                    BuiltinType::Integer(t) => {
                        if let Some(vals) = &t.values {
                            lines = self.compile_int_enum(&tname, vals, false)?;
                            ctx.resolve(&tname)
                        } else {
                            "BigInt".to_string()
                        }
                    ...
                    BuiltinType::CharacterString(t) => match t {
                    ...
                    BuiltinType::Sequence(t) => {
                        ...
                        lines.append(self.compile_struct(
                            &tname,
                            &typ.prefixes,
                            &t.types,
                            ctx,
                    ...
                    BuiltinType::SequenceOf(t) => {
                        ...
                        let (s, mut l) = self.compile_collection_type(t, &inner_ctx)?;
                    ...
                    BuiltinType::Null => "Null".into(),
                    // BuiltinType::Enumerated(t) => {
                    // TODO:
                    // },
                    _ => {
                        "TODO2".to_string()
                        // return Err(format_err!("Unsupported built-in {:?}",
                        // t))
                    }
                })
            }
    ...
    fn compile_value_assign(&self, assign: &ValueAssignment) -> Result<LineBuilder> {
        ...
        let (typename, mut l) = self.compile_type(&name, &assign.typ, &ctx)?;
        ...
        let assign_line = format!("{}: {} = {};", name, typename, wrapped_value);
        if is_complex {
    ...
    fn compile_value(&self, value: &Value, typename: &str, typ: &Type) -> Result<(String, bool)> {
            ...
            Value::Builtin(v) => {
                match v.as_ref() {
                    ...
                    BuiltinValue::Integer(v) => {
                        match v {
                            ...
                            IntegerValue::Identifier(name) => {
                                ...
                                let value_type = self.lookup_value_type(name.as_ref())?;
                    ...
                    BuiltinValue::OctetString(v) => {
                        let val = match v {
                            OctetStringValue::Hex(v) => Self::binary_string(v.as_ref()),
                            OctetStringValue::Bits(v) => Self::binary_string(v.as_ref()),
                        };

                        format!("OctetString::from_static({})", val)
                    }
                    BuiltinValue::Null => "Null::new()".to_string(),
                    BuiltinValue::Sequence(SequenceValue(v)) => {
                        ...
                        for named in v {
                            let field = fields
                                .get(&named.name.as_ref().to_string())
                            ...
                            let (mut val, is_c) =
                                self.compile_value(&named.value, field_tname, &field.typ)?;
                            is_complex |= is_c;
            ...
            l.indented(|l| {
                for (t, tname) in choice.types.types.iter().zip(typenames.iter()) {
                    ...
                    l.indented(|l| {
                        ...
                        l.indented(|l| {
                            l.add(format!("{}::read_der(r_)", tname.replace("<", "::<")));
                            self.compile_type_prefixes(EncodingMode::Read, &t.typ.prefixes, l)
                                .unwrap();
                        });
                        l.add("})?;");
                        l.nl();
        ...
        for a in &body.assignments {
            match a {
                ...
                Assignment::Type(typ) => {
                    if SKIP_ASSIGNMENTS
                        .iter()
                        .find(|v| **v == typ.name.as_ref())
                        .is_some()
                    {
                        println!("Skip type assignment for: {}", typ.name.as_ref());
                        continue;
                    }
