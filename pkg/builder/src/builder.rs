    ...
    async fn build_single_target(
        ...
        match &spec.raw {
            BuildTargetRaw::RustBinary(raw_target) => {
                ...
                let mut rust_flags = target.rustflags().to_string();
                ...
                if !target.target().is_empty() {
                    cmd.arg("--target").arg(target.target());
                }
                ...
                // symlink that we create from 'built-config' to 'built' works correctly.
                {
                    ...
                    create_or_update_symlink(&binary_path, &built_dir_path)?;
                }
...
pub fn create_or_update_symlink<P: AsRef<std::path::Path>, P2: AsRef<std::path::Path>>(
    original: P,
    link_path: P2,
) -> Result<()> {
