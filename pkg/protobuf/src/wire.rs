...
impl WireType {
	fn from_usize(v: usize) -> Result<WireType> {
		Ok(match v {
			...
			4 => WireType::EndGroup,
			5 => WireType::Word32,
			_ => { return Err(err_msg("Invalid wire type number")); }
		})
	}
}
...
impl WireField<'_> {
		...
		while input.len() > 0 {
			...
			let value = match tag.wire_type {
				...
				WireType::LengthDelim => {
					...
					input = rest;
					if input.len() < len { return Err(
						err_msg("Too few bytes for length delimited")); }
					let v = &input[0..len];
					input = &input[len..];
				...
				WireType::EndGroup => {
					...
					let v = match group.take() {
						...
						None => { return Err(err_msg(
							"Saw EndGroup before seeing a StartGroup")); }
					};
	...
	fn serialize(&self, out: &mut Vec<u8>) {
		...
		Tag { field_number: self.field_number, wire_type }.serialize(out);
	...
	pub fn serialize_double(field_number: usize, v: f64, out: &mut Vec<u8>) {
		...
		WireField { field_number, value: WireValue::Word64(&buf) }.serialize(out);
	}
	...
	pub fn serialize_float(field_number: usize, v: f32, out: &mut Vec<u8>) {
		let buf = v.to_le_bytes();
		WireField { field_number, value: WireValue::Word32(&buf) }.serialize(out);
	}
	...
	pub fn serialize_int32(field_number: usize, v: i32, out: &mut Vec<u8>) {
		WireField { field_number, value: WireValue::Varint(v as usize) }
			.serialize(out);
	}

	pub fn parse_int64(&self) -> Result<i64> { Ok(self.value.varint()? as i64) }
	...
	pub fn serialize_fixed32(field_number: usize, v: u32, out: &mut Vec<u8>) {
		...
		WireField { field_number, value: WireValue::Word32(&data) }.serialize(out);
	}
	...
	pub fn serialize_fixed64(field_number: usize, v: u64, out: &mut Vec<u8>) {
		let data = v.to_le_bytes();
		WireField { field_number, value: WireValue::Word64(&data) }.serialize(out);
	}
	...
	pub fn parse_bool(&self) -> Result<bool> { Ok(self.value.varint()? != 0) }
	...
	pub fn parse_string(&self) -> Result<String> {
		...
		String::from_utf8(val)
			.map_err(|_| err_msg("Invalid utf-8 bytes in string"))
	}

	pub fn serialize_string(field_number: usize, v: &str, out: &mut Vec<u8>) {
		WireField { field_number, value: WireValue::LengthDelim(v.as_ref()) }
			.serialize(out);
	}
	...
	pub fn serialize_bytes(field_number: usize, v: &[u8], out: &mut Vec<u8>) {
		WireField { field_number, value: WireValue::LengthDelim(v) }.serialize(out);
	}
	...
	pub fn serialize_enum<E: Enum>(field_number: usize, v: E,
								   out: &mut Vec<u8>) {
	...
	pub fn serialize_message<M: Message>(field_number: usize,
										 m: M, out: &mut Vec<u8>) -> Result<()> {
		let data = m.serialize()?;
		WireField { field_number, value: WireValue::LengthDelim(&data) }
			.serialize(out);
		Ok(())
	}
}
...
macro_rules! enum_accessor {
	($name:ident, $branch:ident, $t:ty) => {
		fn $name(&self) -> Result<$t> {
			...
			else { Err(err_msg("Unexpected value type.")) }
		}
	};
}
