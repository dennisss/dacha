...
static uint16_t min_poe = 0xFFFF;
static uint16_t max_poe = 0;
...
void ADC_Process(void) {
    ...
    uint16_t adc_poe = 0;
    if (g_pcb_revision >= 8) {
        adc_poe = ADC_Read(6);  // PA6 (CH6)
    } else {
        adc_poe = ADC_Read(9);  // PB1 (CH9)
    }
    ...
    // 3. Calculate Normalized POE (Relative to 3.3V)
    // Voltage_Input = (ADC_POE / 4095) * VCC
    // Normalized_ADC = (Voltage_Input / 3300) * 4095
    // Normalized_ADC = ((ADC_POE * VCC) / 4095 / 3300) * 4095
    // Normalized_ADC = (ADC_POE * VCC) / 3300
    uint32_t poe_norm = ((uint32_t)adc_poe * vcc_mv) / TARGET_VREF_MV;
    if (poe_norm > 4095) poe_norm = 4095;
    ...
    if (poe_norm < min_poe) min_poe = poe_norm;
    if (poe_norm > max_poe) max_poe = poe_norm;
}
