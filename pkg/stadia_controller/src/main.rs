...
impl Default for StadiaStickState {
	fn default() -> Self {
		Self { x: AXIS_CENTER, y: AXIS_CENTER, pressed: false }
	}
}
...
fn read_controller() -> Result<()> {
	...
	let product_name = device_handle.read_product_string(
		languages[0], &device_desc, Duration::from_secs(1))?;
	...
	let mut controller = uinput::default()?
		.name(&product_name)?
		.event(DPad::Up)?.event(DPad::Right)?
		.event(DPad::Down)?.event(DPad::Left)?
		.event(GamePad::A)?.event(GamePad::B)?
		.event(GamePad::X)?.event(GamePad::Y)?
		.event(GamePad::TL)?.event(GamePad::TR)?
		.event(GamePad::TL2)?.event(GamePad::TR2)?
		.event(GamePad::ThumbL)?.event(GamePad::ThumbR)?
		.event(GamePad::Start)?.event(GamePad::Select)?
		.event(Misc::_0)?.event(Misc::_1)?.event(Misc::_2)?
	...
	let mut buf = [0u8; 10];
	loop {
		let mut nread = match device_handle.read_interrupt(
			0x83, &mut buf, Duration::new(1, 0)) {
