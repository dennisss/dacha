...
impl<'c> Compiler<'c> {
    pub fn compile(lib: &'c BinaryDescriptorLibrary, runtime_package: &str) -> Result<String> {
        ...
        for s in lib.structs() {
            if !compiler
                .index
                .insert(s.name(), NamedEntity::Struct(s))
                .is_none()
            {
        ...
        for e in lib.enums() {
            if !compiler
                .index
                .insert(e.name(), NamedEntity::Enum(e))
                .is_none()
            {
    ...
    fn compile_type(&self, typ: &Type) -> Result<String> {
        Ok(match typ.type_case() {
            ...
            TypeTypeCase::Named(name) => {
                // TODO: Must verify that this is a valid struct in the file (also sometimes
                // can't be recursive).
                name.to_string()
            }
        ...
        Ok(match typ.type_case() {
            ...
            TypeTypeCase::Buffer(buf) => {
                ...
                match buf.size_case() {
                    ...
                    BufferTypeSizeCase::EndTerminated(_) => {
                        let after_count =
                            after_bytes.ok_or(err_msg("end_terminated buffer not validated"))?;
    ...
    fn compile_serialize_type(
        ...
        Ok(match typ.type_case() {
            ...
            TypeTypeCase::Buffer(buf) => {
                ...
                lines.add(format!(
                    "\t{}",
                    self.compile_serialize_type(buf.element_type(), "item", endian, None)?
                ));
                lines.add("}");
                lines.to_string()
            }
    ...
    // Hello world this is a test of the new keycaps and how well they work compared
    // to the old keycaps. THe anser Is that it's pretty good and there is
    // generally no issues with it so that's great.
    ...
    fn sizeof_type(&self, field_name: &str, typ: &Type) -> Result<Option<SizeExpression>> {
        Ok(match typ.type_case() {
            ...
            // TODO: This will have a well defined length if we can reference a field (unfortunately
            // this is harder for nested fields).
            TypeTypeCase::Buffer(buf) => {
                ...
                let len = match buf.size_case() {
                    BufferTypeSizeCase::FixedLength(len) => SizeExpression::Constant(*len as usize),
                    BufferTypeSizeCase::LengthFieldName(name) => {
    ...
    fn compile_struct(&self, desc: &Struct) -> Result<String> {
        ...
        for field in desc.field() {
            if let TypeTypeCase::Buffer(buf) = field.typ().type_case() {
                if let BufferTypeSizeCase::LengthFieldName(name) = buf.size_case() {
                    ...
                    let size_type =
                        self.compile_type(field_index.get(name.as_str()).unwrap().typ())?;
                    lines.add(format!(
        ...
        ));
        {
            ...
            for field in desc.field() {
                ...
                let mut expr = self.compile_parse_type(
                    field.typ(),
                    desc.endian(),
                    bit_slice,
                    after_bytes,
        ...
        ));
        {
            ...
            for field in desc.field() {
                ...
                let get_parser = |value_expr: &str| {
                    self.compile_serialize_type(field.typ(), &value_expr, desc.endian(), bit_slice)
                };
