...
///
/// Transforms are allowed to be stateful, but should NOT have any asynchronous
/// or non-deterministic behaviors. Rather it is expected that the transform is
/// completely CPU intensive.
///
/// Also note some algorithms only have authentication/checksums validated once
/// all inputs have been read / all outputs have been written. So a corrupt
/// stream may only return an error at the very end. Keep this in mind if doing
/// anything security critical and prefer not to use the output
pub trait Transform {
    ...
    ///   buffered output data is available.
    fn update(
        &mut self,
        input: &[u8],
        end_of_input: bool,
...
pub struct TransformProgress {
    ...
    ///
    /// NOTE: This is allowed to be true even before all inputs have been read.
...
pub fn transform_to_vec(
    ...
    loop {
        ...
        // shot transformation that doesn't require re-allocating.
