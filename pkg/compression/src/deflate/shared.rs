...
/// Maximum distance behind the current output position that a reference can refer to (aka there is no point in using a window buffer larger than this size).
...
pub fn log2(v: usize) -> usize {
	assert!(v > 0);
	(BITS_PER_BYTE * std::mem::size_of::<usize>() - 1)
...
pub fn append_len(len: usize, out: &mut Vec<Atom>) -> Result<()> {
	...
	match len {
		...
		_ => {
			// These are 4 codes per extra bits size, so 2 bits of information are captured by the code that we subtract from the code.
...
pub fn append_distance(dist: usize, out: &mut Vec<Atom>) -> Result<()> {
	...
	if dist <= 4 {
		...
		out.push(Atom::DistCode(code));
		out.push(Atom::ExtraBits(
			BitVector::from_usize(extra, nbits as u8)));
	}
...
pub enum Atom {
	/// A literal/length code from 0-285 to be encoded using the corresponding huffman tree.
	LitLenCode(usize),
	/// A distance code from 0-29 to be encoded using the corresponding huffman code
	DistCode(usize),
...
mod tests {
	...
	fn append_distance_test() {
		...
		assert_eq!(&out, &[
			Atom::DistCode(4),
			Atom::ExtraBits("0".try_into().unwrap())
		]);
