...
impl Inflater {
	...
	pub fn update(&mut self, input: &mut dyn Read /*&[u8]*/,
				  output: &mut [u8]) -> Result<Progress> {
		...
		// TODO: If the input is fully consumed without making any progress, then that would be problematic.
	...
	/// If a BitIoErrorKind::NotEnoughBits is hit, then this operation should be able to be safely rolled back
	fn update_inner(&mut self, strm: &mut BitReader, out: &mut OutputBuffer)
	-> Result<()> {
		self.state = match &mut self.state {
				...
				match header.btype {
					...
					BTYPE_FIXED_CODES => {
						...
						State::CompressedBlockBody {
							litlen_tree, dist_tree, pending_ref: None
						}
					},
			...
			State::UncompressedBlock { num_remaining } => {
				let n = std::cmp::min(out.buf.len() - out.index,
									  *num_remaining as usize);

				let nread = strm.read(
					&mut out.buf[out.index..(out.index + n)])?;
				out.index += nread;
				...
				if new_remaining == 0 {
					...
					State::UncompressedBlock { num_remaining: new_remaining }
				}
			},
			State::DynamicBlockHeader => {
				...
				State::DynamicBlockCodeLenCodeLens {
					hlit, hdist, hclen
				}
			},
			State::DynamicBlockCodeLenCodeLens { hlit, hdist, hclen } => {
				...
				*/
				...
				State::DynamicBlockCodeLens {
					hlit: *hlit, hdist: *hdist, code_len_tree
				}
			},
			State::DynamicBlockCodeLens { hlit, hdist, code_len_tree } => {

				let all_lens = Self::read_dynamic_lens(
					strm, &code_len_tree, *hlit + *hdist)?;
			...
			State::CompressedBlockBody { litlen_tree, dist_tree, pending_ref } => {
				...
				match Self::read_block_codes(
					strm, &litlen_tree, &dist_tree, out)? {
					ReadCodesResult::Done => {
						State::Start
					},
	...
	fn read_dynamic_lens(
		strm: &mut BitReader, code_len_tree: &HuffmanTree, nsymbols: usize) -> Result<Vec<usize>> {
	...
	fn read_block_codes(strm: &mut BitReader, litlen_tree: &HuffmanTree,
					    dist_tree: &HuffmanTree, out: &mut OutputBuffer) -> Result<ReadCodesResult> {
		while out.index < out.buf.len() {
	...
	/// It will read the reference from a combination of the output window and the output buffer.
	fn read_reference(lendist: &LenDist, output_window: &CyclicBuffer,
					  out: &mut OutputBuffer) -> Result<Option<LenDist>> {
		let mut len = lendist.len;
		let dist = lendist.dist;

		// TODO: Implement faster copy (make sure we retain ability to overlap with output)
...
pub trait InflateRead {
	/// 
	/// NOTE: This creates a new Inflater context each time so is not efficient to run multiple times.
