...
impl Host {
	...
	pub fn check_against(&self, request_head: &http::RequestHead) -> bool {
		// TODO: Consider returning a finer grained error to the client if multiple headers are presetn.
	...
	pub fn from_header(v: &[u8]) -> Result<Host> {
		// TODO: Instead assert that they are ASCII (we should make it difficult to write )
		let s: &str = match std::str::from_utf8(v) {
			Ok(s) => s,
			Err(_) => { return Err(err_msg("Invalid header value string")); }
		};
