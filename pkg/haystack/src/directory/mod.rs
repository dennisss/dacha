...
impl Directory {
    ...
    /// Encapsulates picking/load-balancing the next logical volume for writing
    /// to NOTE: We currently assume that all photos are small enough to fit
    /// into a volume such that it will get marked as read-only before any
    /// serious overflows start occuring If there is a failure during
    /// uploading, then it should retry with a new volume
    pub fn choose_logical_volume_for_write(&self) -> Result<LogicalVolume> {
        let volumes = self.db.index_logical_volumes()?;

        let avail_vols: Vec<&LogicalVolume> =
            volumes.iter().filter(|v| v.write_enabled == true).collect();
    ...
    /// TODO: Eventually this needs to be able to pick between multiples caches
    /// per bucket in order to support redundancy in ranges
    pub fn choose_cache(&self, photo: &Photo, vol: &LogicalVolume) -> Result<CacheMachine> {
        ...
        let mut caches = self
            .db
            .index_cache_machines()?
            .into_iter()
            .filter(|m| m.can_read(&self.config))
            .collect::<Vec<_>>();
        ...
        // To pick the cache server, we use a simple Distributed Hash Table approach
        // with a random key per volume
        let mut hasher =
            crypto::sip::SipHasher::default_rounds_with_key_halves(vol.hash_key.flip(), 0);
    ...
    /// Picks a load balanced store machine from which to read the given photo
    /// (to be used only when the cache misses)
    pub fn choose_store(&self, photo: &Photo) -> Result<StoreMachine> {
        let stores = self
            .db
            .read_store_machines_for_volume(photo.volume_id.flip())?
            .into_iter()
            .filter(|m| m.can_read(&self.config))
            .collect::<Vec<_>>();
    ...
    /// Assign the photo to a new logical volume ideally with a blacklist of
    /// machines that we no longer want to use
    ///
    /// TODO: For efficiency, if uploading successfully reaches some machines,
    /// we should prefer to reuse those machines for the next attemp
    pub fn relocate_photo(blacklist: &Vec<MachineId>) {

        // XXX: For updating an existing image, we do need to first retrieve a
        // volume and cookie assignment and then upload, and then commit it
