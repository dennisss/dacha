...
impl DB {
    ...
    pub fn create_param(&self, key: i32, value: Vec<u8>) -> Result<()> {
        ...
        let mut new_param = Param { id: key, value };

        // TODO: Do
        let nrows = match diesel::insert_into(schema::params::table)
            .values(&new_param)
            .execute(&self.conn)
        {
            Ok(n) => n,
    ...
    pub fn read_logical_volume(&self, id_value: VolumeId) -> Result<Option<LogicalVolume>> {
        ...
        Ok(logical_volumes
            ...
            .first::<LogicalVolume>(&self.conn)
            .optional()?)
    }

    /// Find all logical volumes associated with a single machine
    pub fn read_logical_volumes_for_store_machine(
        &self,
        id_value: MachineId,
    ) -> Result<Vec<LogicalVolume>> {
        use super::schema::logical_volumes::dsl::*;
    ...
    pub fn update_logical_volume_writeable(&self, id_value: VolumeId, is: bool) -> Result<()> {
        ...
        expect_changed(
            diesel::update(logical_volumes.filter(id.eq(id_value.flip())))
    ...
    /// Deletes a photo
    /// Will succeed if and only if the logical volume hasn't changed since last
    /// time we checked
    ...
    // NOTE: We would like to atomically increment the size of allocated space as
    // well as adding the volume TODO: Should we also simultaenously change the
    // allocation amounts
    ...
    pub fn create_physical_volumes(
        &self,
        logical_id: VolumeId,
    ...
    // TODO: Eventually may need to be able to delete physical_volume mappings if we
    // decide that a machine is completely dead and not recoverable
    ...
    pub fn read_cache_machine(&self, id_value: MachineId) -> Result<Option<CacheMachine>> {
        ...
        Ok(cache_machines
            ...
            .first::<CacheMachine>(&self.conn)
            .optional()?)
    }

    pub fn update_cache_machine_heartbeat(
        &self,
        id_value: MachineId,
        ready_value: bool,
        addr_ip_value: &str,
        ...
        expect_changed(
            diesel::update(cache_machines.filter(id.eq(id_value.flip())))
                .set((
    ...
    pub fn update_store_machine_heartbeat(
        &self,
        id_value: MachineId,
        ready_value: bool,
        addr_ip_value: &str,
        addr_port_value: u16,
        allocated_space_value: u64,
        total_space_value: u64,
        write_enabled_value: bool,
    ) -> Result<()> {
        use super::schema::store_machines::dsl::*;

        expect_changed(
            diesel::update(store_machines.filter(id.eq(id_value.flip())))
                .set((
                    ready.eq(ready_value),
                    addr_ip.eq(addr_ip_value),
                    addr_port.eq(addr_port_value.flip()),
    ...
    pub fn update_store_machine_health(
        &self,
        id_value: MachineId,
        alive_value: bool,
        healthy_value: bool,
        ...
        expect_changed(
            diesel::update(store_machines.filter(id.eq(id_value.flip())))
                .set((alive.eq(alive_value), healthy.eq(healthy_value)))
                .execute(&self.conn)?,
        )
    }
}
