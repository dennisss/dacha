...
pub enum MachineIds {
    ...
    /// The meaning of this will depend on the method used on the cache but will
    /// generally mean that the cache is free to choose which machines the
...
impl MachineIds {
    pub fn to_string(&self) -> String {
        match self {
            ...
            MachineIds::Data(arr) => arr
                .iter()
                .map(|id| id.to_string())
                .collect::<Vec<String>>()
                .join("-"),
            MachineIds::Unspecified => "-".into(),
        }
    }
}
...
impl CachePath {
    ...
    pub fn from(segs: &[String]) -> std::result::Result<CachePath, &'static str> {
        ...
        Ok(CachePath::Proxy { machine_ids, store })
    }
