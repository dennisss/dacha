...
pub struct MemoryEntry {
    ...
    /// The id of the machine which we got this entry from (used to consistently
    /// rehit the same machine when we want to double check a stale entry)
    pub store_id: MachineId,

    /// The logical volume from which we got this entry from originally
    /// Because photos can change logical volumes upon overflow of their old
    /// ones, this may change over time TODO: Because we don't include this
    /// in the cache key, sequential attempts to lookup different volumes will
    /// cache miss a lot, so ideally we hope that all browsers quickly pick up
    /// the new volume id Additionally because the old volume will typically
    /// reside of a read-only machine, we will typically not end up recaching
    /// old versions
    pub logical_id: VolumeId,

    /// We will opaquely cache most custom and cache-related headers that we get
    /// back from the store (mainly to replicate the original response we got
    /// from the store)
...
pub struct MemoryStore {
    ...
    /// Amount of memory in bytes used up by all cache entries (excluding the
    /// metadata needed to store them)
...
impl MemoryStore {
    ...
    pub fn lookup(&mut self, keys: &NeedleKeys) -> Cached {
        ...
        // But if is only almost stale, we will keep around a reference to it. This way
        // many requests can simultaneously re-up stale references without a full update
        // (small mitigation of the thundering herd issue of cache stale refreshes)
    ...
    /// Explicit removal of an entry (usually if we the cache is the one that
    ...
    fn collect(&mut self) {
        ...
        loop {
            ...
            // NOTE: We will keep stale entries under the assumption that are likely immutable and
            // that on the next wrap of it, it will become
            else if
            /* self.is_stale(&e, &now) || */
            self.need_space() {
                self.delete(&keys, &e);
    ...
    fn is_stale(&self, e: &MemoryEntryInternal, now: &SystemTime) -> bool {
        now.duration_since(e.value.inserted_at)
