...
const VOLUMES_HEADER_SIZE: usize = VOLUMES_MAGIC_SIZE
...
pub struct StoreMachineIndex {
    ...
    file: File, /* TODO: We also want to store the amount of space allocated to each physical
                 * volume and also run a crc over all of the data in this file
                 * considerng tha this is the only real meaningful file, we should ensure that
...
impl StoreMachineIndex {
    ...
    pub fn read_all(&mut self) -> Result<Vec<VolumeId>> {
        self.file
            .seek(SeekFrom::Start(VOLUMES_HEADER_SIZE as u64))?;
