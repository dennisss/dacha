...
/// NOTE: We optimize for a small checksum by only really using it for data
/// integrity on disk and not really security Reasonable security gurantees are
/// produced by assuming that the uploading agent will likely pre-process the
/// file before uploading and updates to the same exact photo aren't really a
/// normal occurence, so most photos are immutable anyway after their first
/// upload aside from automated trusted re-uploads
const CHECKSUM_SIZE: usize = 4;

const FOOTER_MAGIC: &str = "LES!";
const FOOTER_MAGIC_SIZE: usize = 4;

pub const NEEDLE_FOOTER_SIZE: usize = CHECKSUM_SIZE + FOOTER_MAGIC_SIZE;
...
impl NeedleMeta {
    ...
    pub fn occupied_size(&self, block_size: u64) -> u64 {
        ...
        // NOTE: This assumes that needles always start at a block offset, so only need
        // to be aligned based on the size and not the offset in the file
...
pub struct NeedleIndexEntry {
    ...
    // Stored in units of blocks as a u32 similarly to the original paper to keep the memory size
    // small
    pub block_offset: BlockOffset,
}

impl NeedleIndexEntry {
    /// Gets the exact absolute offset in the store file of the start of the
    /// header for this needle
...
pub const NEEDLE_HEADER_SIZE: usize =
    HEADER_MAGIC_SIZE + COOKIE_SIZE + KEY_SIZE + ALT_KEY_SIZE + FLAG_SIZE + SIZE_SIZE;
...
pub const NEEDLE_FLAGS_OFFSET: usize = HEADER_MAGIC_SIZE + COOKIE_SIZE + KEY_SIZE + ALT_KEY_SIZE;
...
impl NeedleHeader {
    ...
    pub fn parse(header: &[u8; NEEDLE_HEADER_SIZE]) -> Result<NeedleHeader> {
        ...
        let mut pos = 4;
        let cookie = &header[pos..(pos + COOKIE_SIZE)];
        pos += COOKIE_SIZE;
        let key = (&header[pos..]).read_u64::<LittleEndian>()?;
        pos += 8;
        let alt_key = (&header[pos..]).read_u32::<LittleEndian>()?;
        pos += 4;
        let flags = header[pos];
        pos += 1;
        let size = (&header[pos..]).read_u64::<LittleEndian>()?;
        pos += 8;
...
/// Backed by a single buffer that is the entire size of the header, data and
/// footer
pub struct Needle {
...
impl Needle {
    /// Reads a single needle at the current position in one read given known
    /// metadata for it
    ...
    pub fn data_bytes(self) -> Bytes {
        self.buf
            .slice(NEEDLE_HEADER_SIZE..(NEEDLE_HEADER_SIZE + self.header.meta.size as usize))
    }
    ...
    ///
    /// If this gives an error, then this physical volume as at least partially
    /// corrupted
    pub fn check(&self) -> Result<()> {
        // TOOD: Insulate all errors so that we can distinguish corruption errors from
        // other errors TODO: Ideally this would all be effectively optional
