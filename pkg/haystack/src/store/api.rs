...
pub const NEEDLE_CHUNK_HEADER_SIZE: usize = size_of::<VolumeId>()
    + size_of::<NeedleKey>()
    ...
    pub fn read_header(reader: &mut dyn Read) -> Result<(NeedleChunkPath, NeedleSize)> {
        ...
        Ok((
            NeedleChunkPath {
                ...
                key,
                alt_key,
                cookie: CookieBuf::from(Bytes::from(cookie)),
            },
            size,
        ))
    }
...
impl From<Bytes> for CookieBuf {
    fn from(data: Bytes) -> CookieBuf {
        CookieBuf { inner: data }
    }
}
...
impl CookieBuf {
    ...
    pub fn to_string(&self) -> String {
        serialize_urlbase64(&self.inner)
            .trim_end_matches('=')
            .to_string()
    }
}
...
impl ETag {
    pub fn from(s: &str) -> Result<ETag> {
        ...
        let volume_id = parts[1]
            .parse::<VolumeId>()
            .map_err(|_| err_msg("Invalid volume id"))?;
        let block_offset = parts[2]
            .parse::<BlockOffset>()
            .map_err(|_| err_msg("Invalid block offset"))?;
        let checksum = parse_urlbase64(parts[3]).map_err(|_| err_msg("Invalid checksum"))?;
    ...
    pub fn matches(&self, other: &ETag) -> bool {
        ...
        // If the other etag came from this machine, then we know for sure if it was
        // modified based on the monotonic offsets (this case should be dominant
        // the majority of the time between the cache and the backend store)
...
/// NOTE: For all paths, aside from the index route of the base path, trailing
/// slashes we consider to be invalid
#[derive(Clone)]
pub enum StorePath {
    /// '/'
    Index,
    ...
    /// '/<volume_id>/<key>'
    Photo { volume_id: VolumeId, key: NeedleKey },
...
impl StorePath {
    pub fn from(segs: &[String]) -> std::result::Result<StorePath, &'static str> {
        ...
        if segs.len() == 1 {
            return Ok(StorePath::Volume { volume_id });
        }
        ...
        if segs.len() == 2 {
            return Ok(StorePath::Photo { volume_id, key });
        }
    ...
    pub fn to_string(&self) -> String {
        match self {
            ...
            StorePath::Volume { volume_id } => format!("/{}", volume_id),
            StorePath::Photo { volume_id, key } => format!("/{}/{}", volume_id, key),
            StorePath::Partial {
                volume_id,
                key,
                alt_key,
