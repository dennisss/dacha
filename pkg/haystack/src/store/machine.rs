...
pub struct MachineContext {
    pub id: MachineId, /* < Same as the id of the 'inst', just externalized to avoid needing to
                        * lock the machine */
    ...
    /// Caches whether or not the store should qualify as 'writeable'. This is
    /// updated in the background thread that does heartbeats
    pub writeable: AtomicBool,
}

impl MachineContext {
    pub async fn from(store: StoreMachine, dir: Directory) -> MachineContext {
        let writeable = store.stats().await.can_write_soft();
...
/// Encapsulates the broad configuration and current state of a single store
/// machine
...
impl StoreMachine {
    ...
    fn open_volume(&mut self, volume_id: VolumeId, expect_empty: bool) -> Result<()> {
        ...
        // Verify that if we opened an existing file, that it wasn't from some other
        // conflicting store
        if vol.superblock.volume_id != volume_id
            || vol.superblock.cluster_id != self.index.cluster_id
        {
    ...
    pub fn create_volume(&mut self, volume_id: VolumeId) -> Result<()> {
        ...
        // care about inconsistencies with empty files with no corresponding index id
        self.index.add_volume_id(volume_id)?;

        Ok(())
    }

    pub async fn stats(&self) -> StoreMachineStats {
        let mut used = 0;
        let mut alloc = 0;
        ...
        let total_space = self.config.store().space()
            - (self.config.store().allocation_size() * self.config.store().allocation_reserved());
    ...
    pub async fn start(mac_handle: &MachineHandle) {
        // TODO: Another duty of this thread will be to start and run compactions (as it
        // has to do the updating of compactions anyway)

        mac_handle
            .thread
            .start(Self::run_background_thread(mac_handle.clone()))
            .await;
    }

    async fn run_background_thread(mac_handle: MachineHandle) {
        let mut pending_alloc = false;

        // TODO: On Ctrl-C, must mark as not-ready to stop this loop, issue one last
        // heartbeat marking us as not ready and wait for all active http requests to
        // finish TODO: For allocation events, we do want this loop to be able
        // to be woken up by a write action that causes the used space amount to tip
        // over the max size
        while mac_handle.thread.is_running() {
            let (cur_should_alloc,) = {
                let (stats, port) = {
                    let mac = mac_handle.inst.read().await;
                    (mac.stats().await, mac.port)
                };

                // TODO: We should just acquire ownership of the directory, as this is the only
                // thread that will actually ever use it (aside from the main thread which
                // probably would like to reclaim ownership of the directory for shutdown)
                ...
                // Hence why read-only machine access would be useful as it rarely ever needs to
                // change
            ...
            if pending_alloc {
                // If we still need to allocate (meaning that no other machine tried allocating
                // since we last checked), then we will proceed with allocating
                if cur_should_alloc {
                    ...
                    // Should should generally mean that at least 2 more cycles to perform
            ...
            // Because many machines can be pending allocation all at once, we will randomly
            // sleep some fraction of the heartbeat before trying to create a volume in
            // order to avoid many machines trying to
            if pending_alloc {
        ...
        if let Err(e) = StoreMachine::shutdown(mac_handle).await {
            eprintln!("Failed during node shutdown: {:?}", e);
        }
    }

    // For shutting down inside of the thread (should consume the thread's
    // mac_handle)
    async fn shutdown(mac_handle: MachineHandle) -> Result<()> {
        let mac = mac_handle.inst.read().await;
        let dir = mac_handle.dir.lock().await;
        ...
        let vols = dir
            .db
            .read_logical_volumes_for_store_machine(mac_handle.id)?;
        for v in vols {
            dir.db
                .update_logical_volume_writeable(v.id as VolumeId, false)?;
        }

        // Perform final heartbeart to take this node off of the ready list
        // The main thing being that we don't really want this blocking with a hold of
        // the machine
        StoreMachine::do_heartbeat(&mac_handle, mac.port, &mac.stats().await, &dir, false)?;
    ...
    /// Check that if for all volumes that exist on this machine, if any of them
    /// are close to being empty, then we need to mark them as being read-only
    /// remotely TODO: We could probably split this entirely once we mark
    /// every single volume as read-only until other constraints change
    fn check_writeability(
        mac_handle: &MachineHandle,
        stats: &StoreMachineStats,
        dir: &Directory,
    ) -> Result<()> {
    ...
    /// Assuming that we want to, this will create a new volume on this machine
    /// and pick other replicas to go along with it
    async fn perform_allocation(mac_handle: MachineHandle) -> Result<()> {
        let num_replicas = mac_handle.config.store().num_replicas() as usize;

        let (mut macs, vol) = {
            let dir = mac_handle.dir.lock().await;

            let all_macs = dir.db.index_store_machines()?;

            let mut macs = all_macs
                .into_iter()
                .filter(|m| m.can_allocate(&dir.config) && (m.id as MachineId) != mac_handle.id)
                .collect::<Vec<_>>();

            if macs.len() < num_replicas - 1 {
                return Err(err_msg(
                    "Not enough replicas available to allocate new volume",
                ));
            }

            let vol = dir.create_logical_volume()?;
        ...
        // TODO: Possibly more useful to spear across less-allocated machines first
        // (with randomness still though)
        let mut rng = clocked_rng();
        rng.shuffle(&mut macs);

        // TODO: Should retry once for each machine
        // Also, if a machine fails, then we can proceed up to the next available
        // machine (next in our random sequence) Basically using the vector as a
        // stream and take until we get some number of successes (but all in parallel)
        // NOTE: We assume that NUM_REPLICAS is always at least 1
        let n_other = if num_replicas > 1 {
            num_replicas - 1
        } else {
            0
        };
        ...
        let arr = macs[0..n_other]
            ...
            .map(|m| async move {
                ...
                let req = http::RequestBuilder::new()
                    ...
                    .path(StorePath::Volume { volume_id: vol_id }.to_string())
                ...
                if !resp.ok() {
                    return Err(format_err!(
                        "Failed to create volume on replica store #{}",
                        vol_id
                    ));
                }
        ...
        all_ids.extend(macs[0..n_other].iter().map(|m| m.id as MachineId));

        // Create all physical mappings
        dir.db.create_physical_volumes(vol_id, &all_ids)?;

        // Mark as writeable
        dir.db.update_logical_volume_writeable(vol_id, true)?;
...
pub struct StoreMachineVolumeStats {
    ...
    /// Total amount of space on disk commited towards this volume but not
...
/// Represents a single snapshot of the store's space usage/allocations at one
/// point in time
pub struct StoreMachineStats {
    // Mainly copied from the store machine for convenience
    pub config: ConfigRef,

    /// The total amount of space that we are allowed to allocate towards
    /// primary data files
    pub total_space: u64,
...
impl StoreMachineStats {
    ...
    /// While the machine is not fully allocated, we will ensure that the
    /// machine never goes above 70% space utilization NOTE: Currently this
    /// will require a lock on all volumes unfortunately
    ...
    /// Whether or not any value on this machine can be written within a
    /// reasonable margin of error NOTE: Machines with no volumes on them
    /// are NOT writeable currently (so we can't immediately accept uploads to a
    /// new volume until all machines have heartbeated again)
