...
/// Simple wrapper around a read needle including the offset into the file
/// (useful for etags)
...
// TODO: We'd also like to be able to set an entire physical volume as
// write_enabled
// - Mainly useful so that we can report it back to clients and so that next
//   time we need to broadcast that we are out of space, we only need to mark
//   volumes which we haven't yet marked as disabled

/// Represents a single file on disk that consists of many photos as part of
/// some logical volume
pub struct PhysicalVolume {
    ...
    /// The length of the file (or the offset to the very end of the last needle
    /// + padding) Because of the potential for partial writes, we won't
    /// trust the size reported on disk after the volume is fully loaded
    extent: u64,

    /// Amount of space allocated in the FS for this file (should be >= the
    /// current size of the file)
    preallocated: u64,
}

impl PhysicalVolume {
    // I need to know the: store directory, volume id, and the cluster_id to make
    // this store
    ...
    pub fn open(config: ConfigRef, path: &Path) -> Result<PhysicalVolume> {
        ...
        let idx = if idx_path.exists() {
            ...
            if i.superblock.cluster_id != superblock.cluster_id
                ...
                || i.superblock.volume_id != superblock.volume_id
            {
        ...
        // Initially starts right after the superblock because we haven't checked any of
        // the needles after it yet
    ...
    /// Lists the size of all space currently being used by this volume and any
    /// associated index This will essentially be the total storage cost of
    /// this volume not containing lower-level filesystem metadata
    pub fn used_space(&self) -> u64 {
        // TODO: May be slightly off as we don't immediately truncate the file after
        // failed writes or extra data at the end of it (as we'd rather try to avoid
        // truncatating pre-emptively in-case a human wants to )
        self.extent + self.index_file.used_space()
    }

    /// Internal utility for adding to the index
    /// This should be atomic w.r.t in-memory datastructures as long as it
    /// doesn't panic
    fn add_to_index(
        &mut self,
        keys: NeedleKeys,
        entry: NeedleIndexEntry,
        ...
        if let Some(old_val) = self.index.get(&keys) {
            if old_val.block_offset == entry.block_offset {
                // This isn't really problematic, but does indicate that we are doing something
                // wrong
    ...
    /// Scans all of the needles in the file and builds the initial index from
    /// them
    ///
    /// (this should generally only be used if no separate index file is
    /// available)
    ///
    /// TODO: We should also use this for checking the integrity of an existing
    /// file
    fn scan_needles(&mut self) -> Result<()> {
        // Start scanning at last known good end of file
        let mut off = self.extent;

        // Start by taking all entries from the condensed index file and seeking to the
        // end of those
        ...
        if size == off {
            ...
            // Truncating to the end of the last file (we will just overwrite the existing
            // data when we start appending more data)
    ...
    /// See what the offset of a needle is as fast as possible (mainly a cache
    /// optimization for etags received upstream from the cache)
    pub fn peek_needle_block_offset(&self, keys: &NeedleKeys) -> Option<BlockOffset> {
        self.index.get(keys).map(|e| e.block_offset)
    }

    // TODO: If we want to go super fast, we could implement the data as a stream
    // and start sending it back to a user right away
    ...
    pub fn read_needle(&mut self, keys: &NeedleKeys) -> Result<Option<NeedleWithOffset>> {
        // This is basically the matter of reading from the current position in the
        // thing
        ...
        self.file.seek(io::SeekFrom::Start(
            entry.offset(self.superblock.block_size),
        ))?;
        ...
        // Separate index files do not persist deletes, so we will be double check the
        // main flags
    ...
    // TODO: We will likely also want to have a create operation that gurantees that
    // a needle does not exist
    /// Adds a new needle to the very end of the file (overriding any previous
    /// needle for the same keys)
    ///
    /// TODO: Probably most useful to return a reference to the full needle
    /// entry
    pub fn append_needle(
        ...
        // Typically needles will not be overwritten, but if they are, we consider
        // needles with the same exact keys/cookie to be identical, so we will ignore
        // attempts to update them TODO: The main exception to this will be
        // error correction (in which case we to be able to do this)
        if let Some(existing) = self.index.get(&keys) {

            // TODO: We can no longer do deduplication of uploads

            // TODO: This now incentivizes making sure that parsing of needle
            // headers is efficient and doesn't do as many copies
        ...
        // TODO: Instead we should be tracking the end as the offset after the last
        // known good needle (as we don't want to compound corruptions)
        let off = self.extent;
        self.file.seek(io::SeekFrom::Start(off))?;
        ...
        let mut next_extent =
            off + (header.len() + (meta.size as usize) + NEEDLE_FOOTER_SIZE) as u64;
        let rem = block_size_remainder(self.superblock.block_size, next_extent);
        next_extent = next_extent + rem;

        // TODO: Should we reject needles that go over the allocation size right here?
        // (currently it is only enforced in the routes layer before the needle is
        // written)

        // Take control of the filesystem allocation process so long as we are not
        // hitting our overall filesystem limit TODO: Another optimization would
        // be preallocate a large amount of space all at once when we are doing
        // compactions as they have a pretty well known size
        if next_extent > self.preallocated {
            ...
            // Using this measurement, the remainder of the space should be left for the
            // data file
            ...
            if next_extent > next_preallocated {
                // TODO: Near the end of the volume, this condition may get hit a lot and result
                // in many small inefficient allocation
        ...
        // Pretty much io::copy but with the output split into making the hash and
        // writing to the output io::copy(&mut data, &mut self.file)?;
        let mut nread: usize = 0;
        while nread < (meta.size as usize) {
            let left = (meta.size as usize) - nread;

            // TODO: If we get a source error, we should probably still truncate our store
            // file to avoid further corruption
        ...
        if nread != (meta.size as usize) {
            ...
            // TODO: Another consideration is that for a stream that is a single file, it
            // needs to be at the end of the file now
    ...
    pub fn predicted_index_size(&self) -> u64 {
        ...
        // When the index file is big enough that actual entries overweight the size of
        // the header metadata, we will try forward predicting the size of the index
        // file
        if self.num_needles() > 128 {
            // Based on the current index-space to data-space ratio, calculate how large we
            // expect the index file to be near max capacity
            ...
            if index_percent > 0.05 {
                ...
                let index_predicted_size =
                    (index_percent * (self.superblock.allocated_space as f64)) as u64;

                // Sanity check the measurement (we should never predict less space than is
    ...
    pub fn delete_needle(&mut self, keys: &NeedleKeys) -> Result<()> {
        ...
        // TODO: Whenever we see a deleted needle, we can add it to our pending
        // compaction count
    ...
    /// Flushes the volume such that any recent append_needle operations persist
    /// to disk
    pub fn flush(&mut self) -> Result<()> {
        // TODO: If we were really crazy about performance, we could count how many
        // needles not yet flushed and perform a flush only if everything isn't already
        // flushed
        self.file.sync_data()?;
        Ok(())
    }
    ...
    pub fn close(mut self) -> Result<()> {
        // NOTE: In general, this should always have been already handled by someone
        // else
        self.flush()?;
