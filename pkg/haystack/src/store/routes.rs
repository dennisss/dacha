...
pub async fn handle_request(
    ...
    // Because ip addresses and ports can change across restarts, we will always
    // verify the request based on a standard hostname pattern derived by this
    // machine's exact id
    ...
    match params {
        ...
        StorePath::Partial {
            ...
            http::Method::GET => {
                read_photo(&req.head, mac_handle, volume_id, key, alt_key, None).await
            }
        ...
        StorePath::Needle {
            ...
            http::Method::GET => {
                read_photo(&req.head, mac_handle, volume_id, key, alt_key, Some(cookie)).await
            }
            http::Method::POST => {
                let content_length = match req.body.len() {
                    Some(0) | None => {
                        return Ok(text_response(
                            http::status_code::LENGTH_REQUIRED,
                ...
                super::route_write::write_single(
                    mac_handle,
                    volume_id,
                    key,
                    alt_key,
...
async fn index_volumes(mac_handle: &MachineContext) -> Result<http::Response> {
...
async fn read_volume(mac_handle: &MachineContext, volume_id: VolumeId) -> Result<http::Response> {
    // NOTE: WE only really need this for long enough to acquire
    let mac = mac_handle.inst.read().await;

    match mac.volumes.get(&volume_id) {
        Some(v) => Ok(json_response(
            http::status_code::OK,
            &physical_volume_to_read_response(&*v.lock().await),
        )),
...
async fn create_volume(mac_handle: &MachineContext, volume_id: VolumeId) -> Result<http::Response> {
    ...
    if !stats.can_allocate() {
        return Ok(text_response(
            ...
            "Can not currently allocate volumes",
        ));
    }
...
async fn read_photo(
    ...
    let given_etag = match req_head.headers.get_one("If-None-Match")? {
        Some(v) => match ETag::from_header(v.value.as_bytes()) {
            Ok(e) => Some(e),
    ...
    // Under the condition that we are using a priveleged route (requesting without
    // a cookie), we will allow checking based solely on the etag value (this will
    // be used exclusively if the cache already has a potential value and just needs
    // us to validate it)
        ...
        if let Some(ref e) = given_etag {
            ...
            if let Some(offset) = off {
                if e.partial_matches(mac_id, volume_id, offset) {
                    // TODO: This response must always be as close possible to the actual response
                    // we would give lower both in this function (- the body)
    ...
    if let Some(c) = given_cookie {
        ...
        // NOTE: In some privileged cases, the cache will request the resource
        // without the cookie
    }
    ...
    // The etag is mainly designed to make hits to the same machine very efficient
    // and hits to other machines at least able to notice after a disk read
    let etag = ETag {
        store_id: mac_id,
        volume_id,
        block_offset: offset,
        checksum: Bytes::from(n.crc32c()),
    };
    ...
    // TODO: If we were obsessed with performance, we would do this on the cache
    // server to avoid transfering it
    {
        let data = n.data();
        if data.len() > 4 {
