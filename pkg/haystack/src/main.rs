...
enum ArgsCommand {
    #[arg(
        name="store",
        desc="Start a store layer machine"
    )]
    Store(StoreCommand),

    #[arg(
        name="cache",
        desc="Starts an intermediate caching layer machine"
    )]
    Cache(CacheCommand),
        ...
        ArgsCommand::Client(cmd) => {
            ...
            match cmd {
                ...
                ClientCommand::ReadUrl(cmd) => {
                    let url = c.read_photo_cache_url(&NeedleKeys { key: cmd.key, alt_key: cmd.alt_key }).await?;
                    println!("{}", url);
                }
            }
        }
