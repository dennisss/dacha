...
# Temp directory in which we will use to store the files we will inject into the CM4. 
PAYLOAD_DIR="/tmp/dacha/rpi_eeprom_payload"

BASE_EEPROM="$SUBMODULE_DIR/recovery/pieeprom.original.bin"
...
cp "$SUBMODULE_DIR/recovery/bootcode4.bin" "$PAYLOAD_DIR/"
