...
#[derive(Clone, Debug)]
#[repr(transparent)]
pub struct QuantizationTable(pub [u8; BLOCK_SIZE]);

/// Base luminance quantization table used as the default table for quality 50
/// (out of 100).
///
/// From ITU T.81, Table K.1.
const BASE_LUMINANCE_QUANTIZATION_TABLE: QuantizationTable = QuantizationTable([
...
const BASE_CHROMINANCE_QUANTIZATION_TABLE: QuantizationTable = QuantizationTable([
...
pub fn create_quantization_tables(quality: usize) -> (QuantizationTable, QuantizationTable) {
...
fn scale_quantization_table(base_table: &mut QuantizationTable, quality: usize) {
    ...
    for v in base_table.0.iter_mut() {
...
mod tests {
    ...
    fn lossless_quantization() {
        ...
        println!("{:?}", create_quantization_tables(95));
    }
}
