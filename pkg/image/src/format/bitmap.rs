...
impl Bitmap {
    ...
    */
    ...
    pub fn parse(file: &[u8]) -> Result<Bitmap> {

        // At least 
        if file.len() < 14 + 4 {
            return Err(err_msg("File too small for Bitmap header"));
        }

        let header = &file[0..14];
        ...
        if !found_magic {
            return Err(err_msg("Failed to recognize Bitmap magic bytes"));
        }

        let file_size = u32::from_le_bytes(*array_ref![header, 2, 4]) as usize;
        if file.len() != file_size {
            return Err(format_err!("Bitmap file is not the right size: {} vs {}", file.len(), file_size));
        }

        let start_off = u32::from_le_bytes(*array_ref![header, 10, 4]) as usize;

        // println!("File size: {}\tStart offset: {}", file_size, start_off);

        let dib_header_size = u32::from_le_bytes(*array_ref![file, 14, 4]) as usize;
        if dib_header_size < 4 {
            return Err(err_msg("DIB header too small"));
        }

        // println!("DIB header size: {}", dib_header_size);

        // NOTE: The header size includes the header size u32
        let dib_header_start = 14 + 4;
        let dib_header_end = dib_header_start + dib_header_size - 4; 

        if file.len() < dib_header_end {
            return Err(err_msg("Bitmap too short for DIB header"));
        }

        let dib_header = &file[dib_header_start..dib_header_end];
        ...
        // BITMAPINFOHEADER
            ...
            let data_size = (header.width_px * header.height_px * 3) as usize;

            let end_off = start_off + data_size;
            if file.len() < end_off {
                return Err(err_msg("File too small for all pixel data"));
            }

            let mut arr = Array::<u8> {
                shape: vec![header.height_px as usize, header.width_px as usize, 3],
                data: file[start_off..end_off].to_vec(),
            };
        ...
        // BITMAPV4HEADER
            ...
            return Err(err_msg("BITMAPV4HEADER not supported"));

        } else {
            return Err(format_err!("Unsupported header of size: {}", dib_header_size));
        }
