...
impl ConsensusModule {
	...
	pub async fn read_index(&self, time: Instant)
						...
						Some(t) => {
							if t == self.meta.current_term {
								return Err(ReadIndexError::RetryAfter(LogPosition {
									index: idx, term: self.log.term(idx).await.unwrap()
								}));
							}
						}
						None => {
	...
	*/
	...
	pub async fn propose_command(&mut self, data: Vec<u8>, out: &mut Tick)
		-> ProposeResult {
	...
	*/
	...
	pub async fn propose_entry(&mut self, data: LogEntryData, out: &mut Tick)
		-> ProposeResult {
...
//	fn cycle_boxed(&mut self, tick: &mut Tick) -> Pin<Box<dyn Future<Output=()> + Send>> {
...
//	}
	...
	pub async fn cycle_impl(&mut self, tick: &mut Tick) {
		...
		if self.config.value.members.len() == 0 ||
			self.config.value.members.get(&self.id).is_none() {
		...
		enum ServerStateSummary {
			...
			Candidate {
				vote_count: usize, election_start: Instant,
		...
		match summary {
			ServerStateSummary::Follower { elapsed, election_timeout } => {
				if !self.can_be_leader().await {
					if self.config.value.members.len() == 1 {
						...
						// progress
						panic!("Corrupt log in single-node mode will not allow \
			...
			ServerStateSummary::Candidate {
				vote_count, election_start, election_timeout
			} => {
				...
				if vote_count >= majority {
					...
					if self.meta.commit_index < last_log_index {
						self.propose_noop(tick).await.expect(
							"Failed to propose self noop as the leader");
					}
			...
			ServerStateSummary::Leader { next_commit_index } => {
				...
				if self.config.value.members.len() +
					self.config.value.learners.len() == 1 {
	...
	async fn find_next_commit_index(&self, s: &ServerLeaderState)
		-> Option<u64> {
	...
	pub async fn append_entries_callback(
		&mut self, from_id: ServerId, last_index: u64,
	...
	pub async fn append_entries_noresponse(&mut self, from_id: ServerId,
									 tick: &mut Tick) {
	...
	pub async fn pre_vote(&self, req: RequestVoteRequest)
		-> RequestVoteResponse {
	...
	pub async fn request_vote(&mut self, req: RequestVoteRequest, tick: &mut Tick)
	...
	// trivial right?
	pub async fn append_entries(&mut self, req: AppendEntriesRequest,
								tick: &mut Tick)
		...
		match self.state {
			...
			ServerState::Candidate(_) => {
				return Err(err_msg(
					"How can we still be a candidate right now?"));
			}
		};
		...
		if first_new < req.entries.len() {
			...
			if next.pos.index != last_log_index + 1 ||
				next.pos.term < last_log_term {
				...
				return Err(err_msg("Next new entry is not immediately after our last local one"));
			}
		}
		...
		let mut last_new_seq = self.log.entry(last_new).await
			.map(|(_, s)| s).unwrap_or(LogSeq(0));
		...
		Ok(FlushConstraint::new(
			response(true, if last_log_index != last_new {
				Some(last_log_index) } else { None }),
