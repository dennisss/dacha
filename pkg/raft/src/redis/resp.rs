...
impl RESPObject {
	...
	/// Converts the object into the format of an incoming client request which should always be an array of bulk strings
	/// TODO: Main issue is that this makes the output less debuggable as it doesn't use the MaybeString type
	pub fn into_command(self) -> std::result::Result<RESPCommand, &'static str> {
		match self {
			RESPObject::Array(arr) => {
				let mut out = vec![];
				...
				for item in arr {
					if let RESPObject::BulkString(data) = item {
						...
						return Err("Some items are not bulk strings");
					}
				}
...
// A wrapper around an array of bytes which might look like to be formatted as a utf8 string
...
impl RESPParser {
	...
	/// If all bytes in the input were not produced, then an object was produced and this function should be called sequentially with the rest of the input
	pub fn parse(&mut self, buf: &[u8]) -> std::result::Result<(usize, Option<RESPObject>), &'static str> {
			...
			let out: Step = match state {
				RESPState::Type => {
					...
					NextState(match c {
						...
						RESP_ERROR => {
							RESPState::String { kind: StringKind::Error, data: vec![] }
						},
						RESP_INTEGER => {
							RESPState::LengthSign(LengthKind::Integer)
						},
						RESP_BULK_STRING => {
							RESPState::LengthSign(LengthKind::BulkString)
						},
						RESP_ARRAY => {
				...
				RESPState::LengthSign(kind) => {
					NextState(if c == ('-' as u8) {
						i += 1;
						RESPState::Length { kind, accum: 0, sign: -1 }
					}
					else {
				...
				RESPState::LengthEnd { kind, accum, sign } => {
					...
					if c != ('\n' as u8) {
						return Err("Missing new line after number")
					}
					...
					match kind {
						LengthKind::Integer => {
							Produce(RESPObject::Integer(val))
						},
						LengthKind::Array => {
							if val < 0 {
				...
				RESPState::String { kind, mut data } => {
					...
					if c == ('\r' as u8) {
						...
						return Err("Received NL before CR");
					}
					else {
				...
				Produce(mut obj) => {
					loop {
						...
						if let Some(mut ent) = stack.pop() {
							...
							// If the array is complete, bubble up with that array as the new production
...
impl tokio::codec::Decoder for RESPCodec {
	...
	fn decode(&mut self, buf: &mut bytes::BytesMut) -> Result<Option<Self::Item>, Self::Error> {
		let (nused, obj) = self.parser.parse(&buf).map_err(|e| {
...
mod tests {
	...
	fn parsing_simple() {
		...
		let cases: Vec<(&'static [u8], RESPObject)> = vec![
			...
			(b"*3\r\n:1\r\n:2\r\n:3\r\n", RESPObject::Array(vec![
				RESPObject::Integer(1), RESPObject::Integer(2), RESPObject::Integer(3)
			])),
			(b"*5\r\n:1\r\n:2\r\n:3\r\n:4\r\n$6\r\nfoobar\r\n", RESPObject::Array(vec![
		...
		for (s, obj) in cases.iter() {
			...
			// In the general case we can't check the serialization containing nils as it may have two forms
			if obj.contains_nil() {
				continue;
			}
