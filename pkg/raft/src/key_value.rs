...
impl StateMachine<KeyValueReturn> for MemoryKVStateMachine {
	...
	fn snapshot(&self) -> Option<StateMachineSnapshot> {

		// For now just copy the 

		// Possibly consider it to have a zero-byte snapshot that generates an empty state machine
		// Alternatively, we can just assume that all snapshots will only be available in memory
		// Complicates in readin them back without a mmemory lock though
		None
	}
