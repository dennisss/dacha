...
/// single step.
pub enum InputValue {
    /// A real non-zero length character value (represents >=1 bytes in the
    /// input search string).
    Character(CharacterValue),

    /// Special zero length symbol that marks a special place in the input
    /// search string.
...
///
/// This can be matched used the Special(_) instruction. Other regular character
/// matching instructions will skip over these symbols until the first non zero
/// length character is seen.
...
pub enum Instruction {
    ...
    /// Similar to 'Char', except matches a special symbol. If a regular
    ...
    /// Schedules execution on the next input at two different program counters
    /// in parallel.
    ///
    /// Effectively this schedules two threads. The first thread is considered
    /// to be 'higher priority' and will execute first and matches from this
    /// thread will be preferred to matches from the second thread.
    ///
    /// TODO: Does this really need two pointers. It will always be the next
    /// instruction + 1 other.
    Split(ProgramCounter, ProgramCounter),

    /// Saves the current position of the string to the string pointers list for
    /// current thread.
    ///
...
impl Instruction {
    pub fn assembly(&self) -> String {
        match self {
            ...
            Instruction::Range { start, end } => {
                format!(
                    "range {} - {}",
                    RegExpSymbol::debug_offset(*start),
                    RegExpSymbol::debug_offset(*end)
                )
            }
    ...
    pub fn codegen(&self) -> String {
        if let Instruction::Special(s) = self {
            return format!(
                "::automata::regexp::vm::instruction::Instruction::Special(
                ::automata::regexp::vm::instruction::SpecialSymbol::{:?})",
                s
            );
        }

        format!(
            "::automata::regexp::vm::instruction::Instruction::{:?}",
            self
        )
    }
}
