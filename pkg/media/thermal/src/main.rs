...
define_arg_command!(Mode {
    ...
    CompressVideoCommand = "compress-video",
    EncodeMP4Command = "encode-mp4",
...
impl RecordCommand {
    ...
    async fn run(self) -> Result<()> {
        ...
        let capture_stream = Self::open_camera().await?;
        ...
        let mut video_writer = VideoWriter::create_new(&self.output_path, VideoWriterOptions {
            deflate: true,
        }).await?;
        ...
        while !cancellation_token.is_cancelled() {
            ...
            for (i, pixel) in value_buffer.chunks(2).enumerate() {
...

                {
                    ...
                    let tt = (t - self.min_temp) * (1.0 / (self.max_temp - self.min_temp));
...
impl CompressVideoCommand {
    async fn run(self) -> Result<()> {
        let mut video_reader = VideoReader::open(&self.input_path).await?;
...
impl EncodeMP4Command {
    async fn run(self) -> Result<()> {
        let mut video_reader = VideoReader::open(&self.input_path).await?;
        let video_file_size = file::metadata(&self.input_path).await?.len();
...
impl DumpStatsCommand {
    async fn run(self) -> Result<()> {
        let mut video_reader = VideoReader::open(&self.input_path).await?;
        video_reader.set_frame_interval(self.frame_interval);
        ...
        out.push_str("time,min_temp,max_temp,p995\n");

        let mut start_time = None;

        while let Some(frame) = video_reader.next().await? {

            let absolute_time = Duration::from_micros(frame.timestamp());

            if start_time.is_none() {
                start_time = Some(absolute_time);
            }

            let time = (absolute_time - start_time.unwrap()).as_secs_f64();
            ...
            for pixel in value_buffer.chunks(2) {
                let v = u16::from_le_bytes(*array_ref![pixel, 0, 2]);
            ...
            all_temps.sort_by(|a, b| a.partial_cmp(b).unwrap());

            out.push_str(&format!("{},{},{},{}\n",
                time, min_temp, max_temp,
                all_temps[(((all_temps.len() as f32) * 0.995) as usize)]
            ));

            tracker.update(video_reader.offset() as usize);
        }

        file::write(&self.output_path, out).await?;
