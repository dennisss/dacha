...
const ROTATION_SCHEDULE: [u8; NUM_ROUNDS] = [
    ...
    fn feistel_function(half_block: &BitVector, sub_key: &BitVector) -> BitVector {
        ...
        let mut substituted_block = BitVector::new();
        {
            ...
            for i in 0..8 {
                ...
                substituted_block = substituted_block.concat(&BitVector::from_lower_msb(vs as usize, 4));
            }
        }
...
mod tests {
    ...
    fn des_initial_final_permute() {
        ...
        for plain in &openssl::des_ecb::PLAIN_DATA {
            ...
            let output = DESBlockCipher::final_permutation(&DESBlockCipher::initial_permutation(&input));
