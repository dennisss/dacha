...
impl EllipticCurve {
    ...
    fn add_points(
        ...
        let s = if p == q {
            m.div(
                &m.add_into(m.mul(&3.into(), &m.pow(&p.x, &2.into())), &self.a),
                &m.mul(&2.into(), &p.y),
            )
        } else {
        ...
        let x = m.sub_into(m.pow(&s, &2.into()), &m.add(&p.x, &q.x));
        let y = m.sub_into(m.mul(&s, &m.sub(&p.x, &x)), &p.y);
...
impl EllipticCurveGroup {
    ...
    pub fn verify_digest_signature(
        ...
        let output_point = self.curve.add_points(
            ...
            &self.curve.scalar_mul(&u_2, &point, &Modulo::new(&self.p)),
            &Modulo::new(&self.p),
        );
    ...
    fn from_bytes(
        ...
        let p = BigUint::from_be_bytes(p_str);
        let a = BigUint::from_be_bytes(a_str);
        let b = BigUint::from_be_bytes(b_str);
        let g_x = BigUint::from_be_bytes(g_x_str);
        let g_y = BigUint::from_be_bytes(g_y_str);
        let n = BigUint::from_be_bytes(n_str);
...
impl MontgomeryCurveGroup<X448> {
    pub fn x448() -> Self {
        let p = BigUint::from(448).exp2() - BigUint::from(224).exp2() - BigUint::from(1);
...
fn cswap(modulo: &Modulo, swap: &BigUint, x_2: BigUint, x_3: BigUint) -> (BigUint, BigUint) {
    if swap.is_one() {
        (x_3, x_2)
    } else {
        (x_2, x_3)
...
mod tests {
    ...
    fn secp256r1_test() {
        ...
        let x = BigUint::from_be_bytes(
            &hex::decode("9EACE8F4B071E677C5350B02F2BB2B384AAE89D58AA72CA97A170572E0FB222F")
                .unwrap(),
        );
        let y = BigUint::from_be_bytes(
            &hex::decode("1BBDAEC2430B09B93F7CB08678636CE12EAAFD58390699B5FD2F6E1188FC2A78")
                .unwrap(),
        );
    ...
    fn x25519_test() {
        assert_eq!(
            BigUint::from_le_bytes(&hex::decode("01").unwrap()).to_string(),
            "1"
        );
        assert_eq!(
            BigUint::from_le_bytes(&hex::decode("0100000002").unwrap()).to_string(),
            "8589934593"
        );
        ...
        let scalar2 = X25519::decode_scalar(
        ...
        let u_in2 = X25519::decode_u_cord(
    ...
    fn ecdsa_test() -> Result<()> {
        ...
        let s = BigUint::from_be_bytes(
            &hex::decode("D12C3FC289DDD5F6890DCE26B65792C8C50E68BF551D617D47DF15A8").unwrap(),
        );
