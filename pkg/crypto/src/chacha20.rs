...
impl ChaCha20 {
	...
	fn new(key: &[u8], nonce: &[u8]) -> Self {
		...
		for i in 0..3 {
			state[13 + i] = u32::from_le_bytes(
				*array_ref![nonce, 4*i, 4]);
		}

		Self { state, bytes_processed: 0 }
	}

	fn quarter_round_with(mut a: u32, mut b: u32, mut c: u32, mut d: u32)
	->(u32, u32, u32, u32) {
		// 1.
		a = a.wrapping_add(b);
		d ^= a;
	...
	fn quarter_round(state: &mut State,
					 x: usize, y: usize, z: usize, w: usize) {
		let (a, b, c, d) = Self::quarter_round_with(
...
mod tests {
	...
	fn chacha20_quarter_round_test() {
		let (a, b, c, d) = ChaCha20::quarter_round_with(
