...
impl<C: BlockCipher> GaloisCounterMode<C> {
	...
	pub fn new(iv: &[u8], cipher: C) -> Self {
		...
		Self { cipher, counter, enc_counter_0, h }
	}
	...
	pub fn encrypt(&mut self, mut plain: &[u8], additional_data: &[u8],
...
impl GHasher {
	pub fn new(h: BigUint) -> Self {
		Self { x: BigUint::zero(), h, field: GaloisField2::gcm128() }
	}
...
mod tests {
	...
	fn gfmul128_test() {
		...
		let a = BigUint::from_be_bytes(&hex::decode(
			"7b5b54657374566563746f725d53475d").unwrap());
		let b = BigUint::from_be_bytes(&hex::decode(
			"48692853686179295b477565726f6e5d").unwrap());
		let c = BigUint::from_be_bytes(&hex::decode(
			"040229a09a5ed12e7e4e10da323506d2").unwrap());
	...
	fn gcm_test() {
		...
		let mut out = vec![];
		let mut gcm = GaloisCounterMode::new(
			&iv, AESBlockCipher::create(&k).unwrap());
