...
/// Gets a lazily initialized reference to a globally shared random number generator.
///
/// This is seeded on the first random generation.
///
/// The implementation can be assumed to be secure for cryptographic purposes but may
/// not be very fast.
///
...
pub trait SharedRng: 'static + Send + Sync {
    ...
    /// Calling generate_bytes after calling reseed with the same seed should always
    ...
    async fn generate_bytes(&self, output: &mut [u8]) {
        {
            ...
            // NOTE: For now we ignore the case of a user requesting a quantity that partially
            // exceeds our max threshold.
            *counter += output.len();
        }
...
///
/// - During initialization and periodically afterwards, we (re-)generate the 256-bit key from a
///   'true random' source (/dev/random).
/// - When a key is selected, we reset the nonce to 0.
/// - The nonce is incremented by 1 for each block we encrypt.
/// - The plaintext to be encrypted is the system time at key creation in nanoseconds.
/// - All of the above are re-seeding in the background every 30 seconds.
/// - Random bytes are generated by encrypting the plaintext with the current nonce and key.
    ...
    pub fn new() -> Self {
        Self {
            state: Mutex::new(ChaCha20RNGState {
                key: [0u8; CHACHA20_KEY_SIZE], nonce: 0, plaintext: [0u8; CHACHA20_BLOCK_SIZE]
            })
        }
    }
}
...
impl SharedRng for ChaCha20RNG {
    ...
    async fn seed(&self, new_seed: &[u8]) {
        ...
        state.plaintext.copy_from_slice(&new_seed[CHACHA20_KEY_SIZE..]);
    }
...
impl<R: Rng + ?Sized> RngExt for R {
    ...
    /// Returns a completely random number anywhere in the range of the number type.
    /// Every number is equally probably of occuring.
    fn uniform<T: RngNumber>(&mut self) -> T {
        let mut buf = T::Buffer::default();
    ...
    /// Limitations:
    /// - 'max' must be >= 'min'.
    /// - For signed integer types for N bits, 'max' - 'min' must fit in N-1 bits.  
    fn between<T: RngNumber>(&mut self, min: T, max: T) -> T {
        let mut buf = T::Buffer::default();
        self.generate_bytes(buf.as_mut());
        T::between_buffer(buf, min, max)
...
impl MersenneTwisterRng {
    ...
    pub fn seed_u32(&mut self, seed: u32) {
        ...
        for i in 1..self.n {
            self.x[i] =
                (self.x[i - 1] ^ (self.x[i - 1] >> (self.w - 2)))
                .wrapping_mul(self.f)
