...
struct GlobalRngState {
    bytes_since_reseed: AsyncMutex<usize>,
    rng: ChaCha20RNG,
}
    ...
    fn new() -> Self {
        Self {
            state: Arc::new(GlobalRngState {
                bytes_since_reseed: AsyncMutex::new(std::usize::MAX),
                rng: ChaCha20RNG::new(),
            }),
        }
    }
    ...
    async fn generate_bytes(&self, output: &mut [u8]) {
        {
            ...
            let mut counter = counter.upgrade().enter();
...
#[async_trait]
impl<R: Rng + Send + 'static> SharedRng for AsyncMutex<R> {
    fn seed_size(&self) -> usize {
        todo!()
    ...
    async fn seed(&self, new_seed: &[u8]) {
        lock!(inner <= self.lock().await.unwrap(), {
            inner.seed(new_seed)
        })
    }
...
pub struct ChaCha20RNG {
    state: AsyncMutex<ChaCha20RNGState>,
}
...
impl ChaCha20RNG {
    ...
    pub fn new() -> Self {
        Self {
            state: AsyncMutex::new(ChaCha20RNGState {
...
impl SharedRng for ChaCha20RNG {
    ...
    async fn generate_bytes(&self, mut output: &mut [u8]) {
        let state = lock!(guard <= self.state.lock().await.unwrap(), {
