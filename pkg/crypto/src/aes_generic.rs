// Helpers for implementing generic non-hardware accelerated AES.

/// Representation of the AES state as a 4x4 matrix.
type StateMat = [[u8; 4]; 4];

// TODO: Compute those at runtime only if we need them.
const AES_S_BOX: [u8; 256] = [
    ,
];

const AES_INV_S_BOX: [u8; 256] = [
    ,
];


const MIX_COLUMNS_MAT: StateMat = [
    [2, 3, 1, 1],
    [1, 2, 3, 1],
    [1, 1, 2, 3],
    [3, 1, 1, 2]
];

const INV_MIX_COLUMNS_MAT: StateMat = [
    [14, 11, 13, 9],
    [9, 14, 11, 13],
    [13, 9, 14, 11],
    [11, 13, 9, 14]
];

/// Used in generating round keys.
pub fn sub_word(w: u32) -> u32 {
    let mut b = w.to_ne_bytes();
    for b_i in b.iter_mut() {
        *b_i = AES_S_BOX[*b_i as usize];
    }

    u32::from_ne_bytes(b)
}

pub fn sub_bytes(state: &mut [u8], invert: bool) {
    let table = if invert { &AES_INV_S_BOX } else { &AES_S_BOX };

    for i in 0..state.len() {
        state[i] = table[state[i] as usize];
    }
}

pub fn shift_rows(state: &mut [u8], invert: bool) {
    for i in 1..4 {
        // Load column i as a u32
        let word = u32::from_be_bytes([
            state[i], state[i + 4*1], state[i + 4*2], state[i + 4*3] 
        ]);

        let new_word = {
            if invert {
                word.rotate_right(8*i as u32)
            } else {
                word.rotate_left(8*i as u32)
            }
        };

        let new_word_bytes = new_word.to_be_bytes();

        // Store back into column 'i'
        for j in 0..4 {
            state[i + 4*j] = new_word_bytes[j];
        }
    }
}

pub fn mix_columns(state: &mut [u8], invert: bool) {
    let state_mat = unsafe {
        &mut *std::mem::transmute::<_, *mut StateMat>(state.as_mut_ptr()) };
    *state_mat = gf_matmul(&if invert { INV_MIX_COLUMNS_MAT } else { MIX_COLUMNS_MAT }, state_mat);
}

/// We assume that 'a' is row-major and 'b' is column-major.
/// Returns a matrix in column-major order.
fn gf_matmul(a: &StateMat, b: &StateMat) -> StateMat {
    let mut out = StateMat::default();
    for j in 0..4 {
        for i in 0..4 {
            let mut v = 0;
            for k in 0..4 {
                v = gf_add(v, gf_mul(a[i][k], b[j][k]));
            }

            out[j][i] = v;
        }
    }

    out
}


/// Multiplies 'a' and 'b' representing polynomials in GF(2^8)
/// with a modulus of 'x^8 + x^4 + x^3 + x + 1'.
///
/// TODO: Use lookup tables instead.
fn gf_mul(mut a: u8, mut b: u8) -> u8 {
    let mut out = 0;

    for i in 0..8 {

        if (b & 1) != 0 {
            out ^= a;
        }

        let overflow_a = (a >> 7) != 0;
        a <<= 1;
        b >>= 1;

        // Subtract 'x^8 + x^4 + x^3 + x + 1' to make 'a' 8-bits again
        // instead of 9 after the shift.
        if overflow_a {
            a ^= 0x1b; 
        }
    }

    out
}

fn gf_add(a: u8, b: u8) -> u8 {
    a ^ b
}
