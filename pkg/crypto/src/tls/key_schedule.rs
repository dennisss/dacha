...
impl KeySchedule {
	pub fn new(hkdf: HKDF, hasher_factory: HasherFactory) -> Self {
		...
		Self { hkdf, hasher_factory, zero_salt, current_salt,
			   empty_transcript_hash, base_keys: None }
	}
	...
	pub fn handshake_secret(&mut self, shared_secret: &[u8]) -> &[u8] {
		...
		self.current_salt = hkdf_expand_label(
			&self.hkdf, &self.current_salt, b"derived",
			&self.empty_transcript_hash, self.hkdf.hash_size() as u16);
	...
	pub fn handshake_traffic_secrets(&mut self, transcript: &Transcript)
		...
		let client_handshake_traffic_secret = hkdf_expand_label(
			&self.hkdf, &self.current_salt, b"c hs traffic",
			&ch_sh_transcript_hash, self.hkdf.hash_size() as u16).into();

		let server_handshake_traffic_secret = hkdf_expand_label(
			&self.hkdf, &self.current_salt, b"s hs traffic",
			&ch_sh_transcript_hash, self.hkdf.hash_size() as u16).into();
	...
	pub fn master_secret(&mut self) -> &[u8] {
		...
		// Master Secret
		self.current_salt = self.hkdf.extract(&self.current_salt,
											  &self.zero_salt);
		&self.current_salt
	}
	...
	pub fn verify_data_server(&self, transcript: &Transcript) -> Bytes {
		...
		let finished_key_server = hkdf_expand_label(
			&self.hkdf, &base_keys.server_handshake_traffic_secret, b"finished", b"",
		...
		// 							 CertificateVerify*))
		self.hkdf.extract(&finished_key_server,
						  &ch_cv_transcript_hash).into()
	}
	...
	pub fn verify_data_client(&self, transcript: &Transcript) -> Bytes {
		...
		let finished_key_client = hkdf_expand_label(
			&self.hkdf, &base_keys.client_handshake_traffic_secret, b"finished", b"",
			self.hkdf.hash_size() as u16);

		let ch_sf_transcript_hash = transcript.hash(&self.hasher_factory);

		self.hkdf.extract(&finished_key_client,
						  &ch_sf_transcript_hash).into()
	}
	...
	pub fn finished(self, transcript: &Transcript) -> FinalSecrets {
		...
		FinalSecrets {
			client_application_traffic_secret_0: hkdf_expand_label(
				&self.hkdf, &self.current_salt, b"c ap traffic",
				&ch_fin_transcript_hash, self.hkdf.hash_size() as u16).into(),

			server_application_traffic_secret_0: hkdf_expand_label(
				&self.hkdf, &self.current_salt, b"s ap traffic",
				&ch_fin_transcript_hash, self.hkdf.hash_size() as u16).into(),

			exporter_master_secret: hkdf_expand_label(
				&self.hkdf, &self.current_salt, b"exp master",
				&ch_fin_transcript_hash, self.hkdf.hash_size() as u16).into(),

			resumption_master_secret: hkdf_expand_label(
				&self.hkdf, &self.current_salt, b"res master",
				&ch_fin_transcript_hash, self.hkdf.hash_size() as u16).into()
		}
	}
...
impl TrafficKeyingMaterial {
	...
	pub fn from_secret(hkdf: &HKDF, aead: &dyn AuthEncAD, traffic_secret: &[u8])
	-> Self {
		...
		let key = hkdf_expand_label(hkdf, traffic_secret, b"key", b"",
									key_length as u16).into();
		let iv = hkdf_expand_label(hkdf, traffic_secret, b"iv", b"",
								   iv_length as u16).into();
	...
	pub fn next_keys(&mut self) -> TrafficKey {
		...
		*array_mut_ref![nonce, nonce.len() - 8, 8] =
		...
		TrafficKey { key: self.base_key.key.clone(), iv: nonce.into() }
	}
}
...
pub fn hkdf_expand_label(hkdf: &HKDF, secret: &[u8], label: &[u8],
						 context: &[u8], length: u16) -> Vec<u8> {
	
	let mut hdkf_label = vec![];
	HkdfLabel { length, label, context }.serialize(&mut hdkf_label);
