...
// TODO: Implement ec_point_formats with just 'uncompressed' being send in client hello
...
impl Extension {
	...
	pub fn serialize(&self, msg_type: HandshakeType,
...
impl ExtensionType {
	...
	fn allowed(&self, msg_type: HandshakeType) -> bool {
		...
		use HandshakeType::*;
		match self {
			server_name => (msg_type == client_hello ||
							msg_type == encrypted_extensions),
			max_fragment_length => (msg_type == client_hello ||
									msg_type == encrypted_extensions),
			status_request => (msg_type == client_hello ||
							   msg_type == certificate_request ||
							   msg_type == certificate),
			supported_groups => (msg_type == client_hello ||
								 msg_type == encrypted_extensions),
			signature_algorithms => (msg_type == client_hello ||
									 msg_type == certificate_request),
			use_srtp => (msg_type == client_hello ||
						 msg_type == encrypted_extensions),
			heartbeat => (msg_type == client_hello ||
						  msg_type == encrypted_extensions),
			application_layer_protocol_negotiation =>
				(msg_type == client_hello || msg_type == encrypted_extensions),
			signed_certificate_timestamp =>
				(msg_type == client_hello || msg_type == certificate_request ||
				 msg_type == certificate),
			client_certificate_type => (msg_type == client_hello ||
										msg_type == encrypted_extensions),
			server_certificate_type => (msg_type == client_hello ||
										msg_type == encrypted_extensions),
			padding => (msg_type == client_hello),
			key_share => (msg_type == client_hello || msg_type == server_hello ||
						  msg_type == hello_retry_request),
			pre_shared_key =>
				(msg_type == client_hello || msg_type == server_hello),
			psk_key_exchange_modes =>
				(msg_type == client_hello),
			early_data => (msg_type == client_hello ||
				msg_type == encrypted_extensions ||
				msg_type == new_session_ticket),
			cookie =>
				(msg_type == client_hello || msg_type == hello_retry_request),
			supported_versions =>
				(msg_type == client_hello || msg_type == server_hello ||
				 msg_type == hello_retry_request),
			certificate_authorities =>
				(msg_type == client_hello || msg_type == certificate),
...
fn parse_supported_versions(input: Bytes, msg_type: HandshakeType)
-> ParseResult<Extension> {
	if msg_type == HandshakeType::client_hello {
		map(SupportedVersionsClientHello::parse,
			|v| Extension::SupportedVersionsClientHello(v))(input)
...
fn parse_key_share(input: Bytes, msg_type: HandshakeType)
-> ParseResult<Extension> {
	match msg_type {
		HandshakeType::client_hello => {
			map(KeyShareClientHello::parse,
				|v| Extension::KeyShareClientHello(v))(input)
		},
		HandshakeType::hello_retry_request => {
			map(KeyShareHelloRetryRequest::parse,
				|v| Extension::KeyShareHelloRetryRequest(v))(input)
		},
