use crate::matrix::{Vector3f, Matrix3f};
...
pub fn to_axis_angle(mat: &Matrix3f) -> Vector3f {
    let trace = mat[(0, 0)] + mat[(1, 1)] + mat[(2, 2)];

    // trace = -1 (rotation is 180 degrees)
    if (trace + 1.0).abs() < 0.001 {
        let x;
        let y;
        let z;
        ...
        let axis = Vector3f::from_slice(&[x, y, z]).normalized();
        return axis * std::f32::consts::PI;
    }
    ...
    if angle.abs() < 0.0001 {
        return Vector3f::from_slice(&[0.0, 0.0, 0.0]);
    }

    let axis = Vector3f::from_slice(&[
        mat[(2, 1)] - mat[(1, 2)],
        mat[(0, 2)] - mat[(2, 0)],
        mat[(1, 0)] - mat[(0, 1)],
    ]).normalized();
...
pub fn from_axis_angle(axis_angle: &Vector3f) -> Matrix3f {
    let angle = axis_angle.norm();
    let axis = axis_angle.clone().normalized();

    let k = Matrix3f::from_slice(&[
        0., -axis.z(), axis.y(),
        axis.z(), 0., -axis.x(),
        -axis.y(), axis.x(), 0.
    ]);
...
///
/// See https://en.wikipedia.org/wiki/Rodrigues%27_rotation_formula
pub fn rotate_by_axis_angle(point: &Vector3f, axis_angle: &Vector3f) -> Vector3f {
    let angle = axis_angle.norm();
...
pub fn rotate_by_axis_angle_near_zero(point: &Vector3f, axis_angle: &Vector3f) -> Vector3f {
