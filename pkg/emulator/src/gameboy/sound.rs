...
impl SoundControllerState {
    ...
    fn compute(&self, data: &mut [f32], sample_index: u64, sample_rate: u64) {
        ...
        for (i, sample) in data.chunks_mut(2).enumerate() {
            ...
            let time = ((sample_index + (i as u64)) as f64) / (sample_rate as f64);
    ...
    fn mix_terminal(&self, terminal: SoundOutputTerminal, channels: &[f32; 4]) -> f32 {
        let mut value = 0.0;
    ...
    fn store8(&mut self, addr: u16, value: u8) -> Result<()> {
        ...
        match addr {
            ...
            0xFF11 => {
                ...
                self.channel1.length_counter = self.channel1.length_duty.length();
            }
            ...
            0xFF16 => {
                ...
                // this will be '64 - value'
                self.channel2.length_counter = self.channel2.length_duty.length();
            }
            ...
            // Channel 3: Wave
            0xFF1A => {
                self.channel3.playing = bitget(value, 7);
            }
            ...
            0xFF22 => {
                self.channel4.polynomial_counter = value;
            }
            0xFF23 => {
...
impl WaveOutputLevel {
    ...
    fn set(&mut self, value: u8) {
        self.register = value & 0b01100000;
    }
    fn get(&self) -> u8 {
        self.register
    }
