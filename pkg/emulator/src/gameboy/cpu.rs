...
impl CPU {
    ...
    pub fn step_full(
        ...
        interrupts: &RefCell<InterruptState>,
    ) -> Result<()> {
    ...
    pub fn step(
        ...
        //			println!("A:{:02x} F:{}{}{}{} BC:{:04x} DE:{:04x} HL:{:04x} SP:{:04x}
        // PC:{:04x} (cy: {})", 					 self.registers.a().get(),
...
macro_rules! cond {
    ($state:ident, NZ) => {
        !$state.cpu.registers.flag_z()
    };
    ($state:ident, Z) => {
        $state.cpu.registers.flag_z()
    };
    ($state:ident, NC) => {
        !$state.cpu.registers.flag_c()
    };
    ($state:ident, C) => {
        $state.cpu.registers.flag_c()
    };
...
// TODO: Verify that all adds and subtracts are wrapping (or is at least
// unchecked)
...
const RESERVED: Instruction = Instruction {
    mnemonic: "RESERVED",
    handler: &|state| Err(err_msg("Executing undefined instruction.")),
};
...
mod tests {
    ...
    fn it_works() {
        ...
        for i in 0..8 {
            ...
            //			println!("{:x?} {}", this.cpu.registers,
            // this.cpu.registers.flag_z());
        }
    }
}
...
// Array.from(document.getElementsByTagName('td')).map((el) => { return
// el.innerText.split('\n')[0]; }).join('\n')
