...
impl MBC3 {
	pub fn new(rom: Vec<u8>) -> Result<Self> {
		...
		Ok(Self {
			rom, rom_number: 1,
			ram, ram_enabled: false, ram_number: 0,
			clock_latch: false,
...
//			println!("LATCH");
			...
			let v = match value {
				...
				_ => { return Err(err_msg(
					format!("Unexpected latch value {}", value))); }
			};
			...
			if self.ram_number <= 0x03 {
				self.ram[RAM_BANK_SIZE*(self.ram_number as usize) + off] =
					value;
			}
			else {
				self.clock_registers[(self.ram_number - 0x8) as usize] =
					value;
			}
		}
...
/// TODO: Move to a separate file as this causes a cyclic reference between memory and video files.
/// TODO: Verify that bits 5, 6, 7 are not set.
#[derive(Default)]
		...
		if self.dma_pending > 0 {
			if addr < 0xff80 || addr > 0xfffe {
				return Err(err_msg(format!(
					"Can only access HRAM during DMA transfer: {:04X}", addr)));
			}
		}
...
//			println!("Serial write {:x} {}", value, value as char);
		...
		if addr == 0xff50 {
			if !self.boot_rom_enabled() {
				return Err(
					err_msg("Attempting to toggle boot ROM after disabled."));
			}
			...
			return Err(err_msg(format!(
				"Storing into unsupported memory range {:04X}", addr)));
		}
	...
	fn load8(&mut self, addr: u16) -> Result<u8> {
		...
		Err(err_msg(format!(
			"Loading from unsupported memory range: {:04X}", addr)))
	}
}
