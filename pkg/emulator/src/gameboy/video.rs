...
const CYCLES_PER_LINE: u64 = OAM_SEARCH_CYCLES + PIXEL_TRANSFER_CYCLES + H_BLANK_CYCLES;
...
impl VideoController {
    ...
    fn draw_line(&mut self, screen_y: u8) -> Result<()> {
        ...
        // Draw background.
        {
            let tile_map = self
                .vram
                .tile_map(self.registers.control.background_tile_map_select());

            let tile_data = self
                .vram
                .tile_data(self.registers.control.background_window_tile_data_select());
            ...
            for screen_x in 0..(SCREEN_WIDTH as u8) {
                ...
                let color_num = tile.get(frame_x - tile_ptr.x, frame_y - tile_ptr.y);
        ...
        if self.registers.control.window_display_enabled() && screen_y >= self.registers.window_y {
            let tile_map = self
                .vram
                .tile_map(self.registers.control.window_tile_map_select());

            let tile_data = self
                .vram
                .tile_data(self.registers.control.background_window_tile_data_select());

            let start_x = if self.registers.window_x >= 7 {
        ...
        if self.registers.control.object_display_enabled() {
            ...
            loop {
                ...
                let pallete = if obj.upper_pallete() {
                    &self.registers.object_pallete1
                } else {
                    &self.registers.object_pallete0
                };
    ...
    pub fn step(&mut self) -> Result<()> {
        ...
        if y < SCREEN_HEIGHT {
            if x == 0 {
                ...
                if coincide_y && self.registers.status.ly_coincidence_interrupt_enabled() {
                    println!("Fire 1");
...
impl MemoryInterface for VideoController {
    fn store8(&mut self, addr: u16, value: u8) -> Result<()> {
        match addr {
            ...
            _ => {
                return self.registers.store8(addr, value);
            }
        }
    ...
    fn load8(&mut self, addr: u16) -> Result<u8> {
        Ok(match addr {
            0x8000..=0x9FFF => self.vram.data[(addr - 0x8000) as usize],
            0xFE00..=0xFE9F => self.oam[(addr - 0xFE00) as usize],
            _ => {
                return self.registers.load8(addr);
            }
        })
    }
}
...
impl MemoryInterface for VideoRegisters {
    fn store8(&mut self, addr: u16, value: u8) -> Result<()> {
        match addr {
            ...
            0xFF44 => {
                panic!("Setting y counter not implemented");
                self.counter_y = value;
            }
            0xFF45 => {
                panic!("Compare y not implemented");
                self.compare_y = value;
            }
            0xFF47 => {
                self.background_pallete.value = value;
            }
            0xFF48 => {
                self.object_pallete0.value = value;
            }
            0xFF49 => {
                self.object_pallete1.value = value;
            }
            0xFF4A => {
                self.window_y = value;
            }
...
impl LcdControlRegister {
    ...
    fn background_window_tile_data_select(&self) -> bool {
        bitget(self.value, 4)
    }
    fn background_tile_map_select(&self) -> bool {
        bitget(self.value, 3)
    }
    /// If true, then sprites are 8x16, otherwise they are 8x8.
    fn object_big(&self) -> bool {
        bitget(self.value, 2)
    }

    fn object_display_enabled(&self) -> bool {
        bitget(self.value, 1)
    }
...
impl LcdStatusRegister {
    ...
    fn ly_coincidence_interrupt_enabled(&self) -> bool {
        bitget(self.value, 6)
    }
...
impl TileMap<'_> {
    fn lookup(&self, x: u8, y: u8) -> TilePointer {
        let offset =
            (((y / TILE_DIMENSION) as u16) * TILES_PER_ROW) + ((x / TILE_DIMENSION) as u16);
...
impl Tile<'_> {
    ...
    fn get(&self, x: u8, y: u8) -> ColorNumber {
        ...
        ColorNumber(bitgetv(self.data[off], bit) | (bitgetv(self.data[off + 1], bit) << 1))
    }
}
